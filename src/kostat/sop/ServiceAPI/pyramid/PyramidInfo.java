...
public class PyramidInfo extends AbsQuery<List> {
	...
	public List executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			}else if(mapParameter.get("gubun").equals("ratio")){
				result = session.selectList("yearRatioData", mapParameter);
				//System.out.println(result); //2017.12.04 [개발팀] 시큐어코딩
			}else if(mapParameter.get("gubun").equals("addData")){
				result = session.selectList("pyramidAddData", mapParameter);
				//System.out.println(result); //2017.12.04 [개발팀] 시큐어코딩
			}else{
				result = session.selectList(getQueryStr(), mapParameter);
			}
