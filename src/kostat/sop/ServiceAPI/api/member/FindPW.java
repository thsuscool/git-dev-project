...
public class FindPW extends AbsQuery<Map> {
	private static final Log logger = LogFactory.getLog(FindPW.class);
	...
	enum MustParam
	{
		member_key, member_id, member_nm, cp_no
	}
	...
	public Map executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			mapParameter.put(MustParam.member_nm.name(), SecureDB.encryptAria256(member_nm));
			mapParameter.put(MustParam.cp_no.name(), SecureDB.encryptAria256(cp_no));
			
			//비밀번호 찾기를 통해 검색한 회원키
			String member_key = (String)session.selectOne("member.getFindPW", mapParameter);
			if(member_key == null) {
