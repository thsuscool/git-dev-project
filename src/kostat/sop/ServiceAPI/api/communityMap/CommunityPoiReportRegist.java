 ...
 * <p/>
 */
public class CommunityPoiReportRegist extends AbsQuery<HashMap<String,Object>> {
	private static final Log logger = LogFactory.getLog(CommunityPoiReportRegist.class);
	...
	public HashMap<String,Object> executeAPI(
		...
		try {
			...
			if(this.validation(login_id, community, poi, mapParameter)){
				...
				resultData.put("success", (int)session.insert("communityPoiSttemnt.insertPoiSttemnt", mapParameter)>0);
			}
	...
	private boolean validation(String login_id, HashMap<String,Object> community, HashMap<String,Object> poi,Map<String,Object> mapParameter) throws Exception,IllegalArgumentException{
		...
		}else if(mapParameter.get("sttemnt_content")==null||mapParameter.get("sttemnt_content").toString().length()<=0){
			throw new ApiException("신고할 내용을 입력해주세요");
		}else if(mapParameter.get("sttemnt_content").toString().length()>1300){
			throw new ApiException("신고할 내용은 최대 1300자까지 작성하실 수 있습니다");
		}
		...
		if(community.get("cmmnty_partcptn_grant_yn").equals("M")&&!community.get("usr_id").equals(login_id)){
			...
			}else if(poi.get("usr_id").toString().equals(mapParameter.get("id").toString())){
				throw new ApiException("본인이 쓴 자료에는 신고하실 수 없습니다");
			}else{
			...
			}else if(poi.get("usr_id").toString().equals(login_id)){
				throw new ApiException("본인이 쓴 자료에는 신고하실 수 없습니다");
			}
		}
		return true;
