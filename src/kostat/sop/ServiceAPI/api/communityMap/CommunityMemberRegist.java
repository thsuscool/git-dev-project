 ...
 * <p/>
 */
public class CommunityMemberRegist extends AbsQuery<HashMap<String,Object>> {
	private static final Log logger = LogFactory.getLog(CommunityMemberRegist.class);
	...
	public HashMap<String,Object> executeAPI(
		...
		try {
			...
			if(this.validation(mapParameter)){
				resultData.put("success", session.insert("communityRegistMember.insertCmmntyMapRegMber",mapParameter)>0);
			}
	...
	private boolean validation(Map<String,Object> mapParameter) throws Exception,IllegalArgumentException{
		...
		if(!community.get("usr_id").equals(login_id)){
			...
			if(session.selectOne("communityRegistMember.selectCmmntyMapRegMber",mapParameter)==null){
				if(community.get("usr_id").equals(mapParameter.get("id").toString())){
					throw new ApiException("개설자 아이디는 등록할 수 없습니다");
				}else if(mapParameter.get("id").toString().length()>30){
					throw new ApiException("아이디는 최대 30자까지 작성하실 수 있습니다");
				}else if(!mapParameter.get("id").toString().matches("[a-z|A-Z|0-9]+")){
					throw new ApiException("아이디는 영문 또는 숫자만 사용하실 수 있습니다");
				}else if(mapParameter.get("pw").toString().matches(".*[ㄱ-ㅎㅏ-ㅣ가-힣]+.*+")){
					throw new ApiException("패스워드는 한글을 포함할 수 없습니다");
				}else if(mapParameter.get("nm")!=null&&mapParameter.get("nm").toString().length()>30){
				...
				throw new ApiException("아이디가 존재합니다.");
			}
		}
	}
}
