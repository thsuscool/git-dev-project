 ...
 * <p/>
 */
public class CommunityMemberApprovalOut extends AbsQuery<HashMap<String,Object>> {
	private static final Log logger = LogFactory.getLog(CommunityMemberApprovalOut.class);
	...
	public HashMap<String,Object> executeAPI(
		...
		try {
			...
			}else if(community.get("regist_yn").equals("Y")||community.get("regist_yn").equals("W")){
				mapParameter.put("approval_distinct","WS");
				resultData.put("success", (int)session.update("communityApproval.updateCmmntyApprovalDistinct",mapParameter)>0);
			}else{
				throw new ApiException("가입하신 소통지도가 아닙니다.");
			}
