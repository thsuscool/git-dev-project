 ...
 * <p/>
 */
public class CommunityCustomSymbolGroupModify extends AbsQuery<HashMap<String,Object>> {
	private static final Log logger = LogFactory.getLog(CommunityCustomSymbolGroupModify.class);
	...
	enum OptionParam{
		customSymbolName,
		customSymbolFile,
		modifyCustomSymbolId,
		modifyCustomSymbolName,
		deleteSymbolId,
		cmmnty_map_id
	}
		...
		try {
			...
			if(hasCount>0){
				...
				if(symbolGroup.get("usr_id").equals(httpSession.getAttribute("member_id").toString())){
					session.update("communityCustomSymbol.updateCustomSymbolGoupNm", mapParameter);//그룹 이름 수정
					...
					String[] modifyCustomSymbolId = req.getParameterValues("modifyCustomSymbolId");
					String[] modifyCustomSymbolName = req.getParameterValues("modifyCustomSymbolName");
					for(int i=0;i<modifyCustomSymbolId.length;i++){
						HashMap<String,Object> modify = new HashMap<String,Object>();
						modify.put("custom_symbol_id",modifyCustomSymbolId[i]);
						modify.put("label_nm",modifyCustomSymbolName[i]);
						session.update("communityCustomSymbol.updateCustomSymbolLabelNm", modify);//심볼 이름 수정
					}
					if(mapParameter.get("deleteSymbolId")!=null){
						String[] custom_symbol_id_list = req.getParameterValues("deleteSymbolId");
						mapParameter.put("custom_symbol_id_list",custom_symbol_id_list);
						session.delete("communityCustomSymbol.deleteCommunityCustomSymbolList",mapParameter);
					}
					if(customSymbolFileList!=null&&customSymbolFileList.size()>0){
						int lastSort = session.selectOne("communityCustomSymbol.selectCustomSymbolLastSort",mapParameter);
						for(int i=0;i<customSymbolFileList.size();i++){//새로운 심볼 등록
							String[] customSymbolName = req.getParameterValues("customSymbolName");
							if(!this.insertCustomSymbolFile(mapParameter.get("custom_symbol_group_id").toString(), customSymbolName[i], filePath, customSymbolFileList.get(i),i+1+lastSort)){
								throw new ApiException("등록을 실패하였습니다");
							}
						}
					}
					List<HashMap<String,Object>> reorder = session.selectList("communityCustomSymbol.selectCustomSymbolReOrder",mapParameter);
					for(HashMap<String,Object> obj : reorder){
						session.update("communityCustomSymbol.updateCustomSymbolReOrder",obj);
					}
				}else{
					throw new ApiException("자신이 등록한 그룹만 수정이 가능합니다");
				}
			}
