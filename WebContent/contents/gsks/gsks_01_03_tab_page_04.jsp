...
<%
	...
	//String version_sel       = lData.get("version_sel");	
	...
	try {
		...
		String sql = " select api_auth_key \n";
						...
						sql += "							, sgis_name , sgis_member_id, api_server_ip, b.api_version 		\n";
						sql += "		) api			\n";
  ...
  <div class="clear"></div>
			...
			<input type="hidden" name="code" value="S">
				...
				<table width="100%" cellpadding="0" cellspacing="1" class="table1">
					...
					<select name="version_sel">
						...
						<option value="1.0" <%=version_sel.equals("1.0")?"selected":"" %>>1.0</option>
						<option value="2.0" <%=version_sel.equals("2.0")?"selected":"" %>>2.0</option>
					</select> --%>
   ...
   <thead>
      <tr>
        ...
        <th class="td_top" width="40">version</th>
	...
	%>
		<tr>
			...
			<td width="40" class="cell_center"><a href="javascript:detailPopView('<%=api_auth_key %>','<%=api_element_id %>')"><%=api_version %></a></td>
	...
	<% rowcnt++; } %>
     ...
     <%if(rowcnt == 0) {%>
     	<tr>
     		<td colspan="8" class="t_end cell_center">No Data</td>
     	</tr>
     <%} %>
<%} catch(Exception e) {
	System.out.print("sgisWebError : ");
