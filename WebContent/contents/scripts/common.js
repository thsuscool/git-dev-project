...
var login_url = "http://"+location.host+"/contents/member/pop_login.jsp";
...
function decodeUTF8(str) {
  ...
  function pop_loginClicked() {
    if(document.loginFm.userid.value == "") {
      ...
      //document.loginFm.action="https://sgis1.kostat.go.kr/contents/member/pop_login_step.jsp"; //실환경용
      //document.loginFm.action="https://211.34.86.29/contents/member/pop_login_prc.jsp"; //개발서버용
      document.loginFm.action="http://localhost:8080/contents/member/pop_login_prc.jsp"; //개발용 20140508
...
//pop_join
// 130410
// context가 sgisnavigator 로 요청할 경우 분기 처리 - 정재원
  function pop_joinClicked() {
		location.href= login_url+"?re_Url=/vname_input_seed_mem.jsp"; 
  }
  ...
  function pop_searchIdPwClicked() {
	   location.href= login_url+"?re_Url=/contents/member/search_idpwd.jsp";
  }
...
function link(url){
  window.open(url, '', 'toobar=0,status=0,fullscreen=0,menubar=0,scrollbars=0,resizable=yes');
}
function link2(url){
