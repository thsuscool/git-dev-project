...
//		html1 += '<select id="sidoSelect_1" style="border: 0px;background-color: transparent;padding-bottom: 6px;"></select></span>'; 
//		html1 += '<select id="sggSelect_1" style="visibility: hidden; border: 0px;background-color: transparent;padding-bottom: 6px;"></select>';
...
//		html2 += '<select id="sidoSelect_2" style="border: 0px;background-color: transparent;padding-bottom: 6px;"></select></span>'; 
//		
//		$(window).resize();
...
//			$thematicMap.rightMap.commonMoveEvent();
	...
	$thematicMap.Api = function () {
		var that = this;
		this.getThemeKosisDataList = function (target) {
			var options = {
				target : target
			};
			$statsPotal.api.thematicMap.getThemeKosis({
				...
				success : function (status, res) {
					that.successKosisData(res, options)();
				}
			});
		};
		// 남한경계
		...
		this.successKosisData = function (res, options) {
			return function () {
				if (res.errCd === 0) {
					console.log('options ' + options.target.mapId + 'successKosisData', res.result.kosisInfo);
					options.target.staticJsonObj.push(res.result.kosisInfo);
					
					if (options.target.mapId == 'leftMap') {
						options.target.formatter.combinStatsToGeoJon(options.target.geoJsonObj, '(kg/년)', 'chemicals_discharge_init');
					}
					else {
						options.target.formatter.combinStatsToGeoJon(options.target.geoJsonObj, '(kg/년)', 'chemicals_discharge_cur');
						...
						var logOptions = {
							...
							params : {
								...
								map : $thematicMap.leftMap.map
							}
						}
						//API로그 쌓기 (행정동코드로 지역 조회)
						addrCdToNm("2012", "00", logOptions);
					}
