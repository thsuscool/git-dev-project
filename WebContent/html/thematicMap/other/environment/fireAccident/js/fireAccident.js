...
//				}
	...
	$thematicMap.request = {
		...
		getThemaFireAccident : function () {
			$statsPotal.api.thematicMap.getThemaFireAccident({
				method : 'POST',
				success : $thematicMap.response.successThemaFireAccident
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				$thematicMap.request.getThemaFireAccident();
			});
	...
	$thematicMap.response = {
		...
		successThemaFireAccident : function (status, res) {
			if (res.errCd === 0) {
				console.log('successSingleFamily', res.result);
				$thematicMap.staticJsonObj.push(res.result.fireAccidentInfo);
