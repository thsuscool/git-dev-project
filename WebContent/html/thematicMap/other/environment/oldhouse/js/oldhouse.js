...
//			window.parent.$mapNavigation.event.setUIEvent();
			...
			// 가락동좌표
//			$thematicMap.ui.createMap([ 966621, 1944048 ], 7);
...
//				window.parent.$mapNavigation.ui.sidoSelectSet(1); 
	...
	$thematicMap.request = {
		...
		getThemeoldHouseList : function (base_year, adm_cd) {
			$statsPotal.api.thematicMap.getThemeoldHouse({
				param : {					
					adm_cd : adm_cd
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successThemeoldHouse
			});
		},		
		// 남한경계
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				...
				$thematicMap.request.getThemeoldHouseList($thematicMap.ui.bnd_year, adm_cd);
	...
	$thematicMap.response = {
		...
		successThemeoldHouse : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successThemeoldHouse', res.result.oldhouseInfo);
				$thematicMap.staticJsonObj.push(res.result.oldhouseInfo);
				console.log('$thematicMap.geoJsonObj', $thematicMap.geoJsonObj);
				$thematicMap.formatter.combinStatsToGeoJon($thematicMap.geoJsonObj, '개', 'house_cnt');
