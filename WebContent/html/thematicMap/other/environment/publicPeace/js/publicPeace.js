...
//			window.parent.$mapNavigation.event.setUIEvent();
			...
			// 가락동좌표
//			$thematicMap.ui.createMap([ 966621, 1944048 ], 10);
...
//				$thematicMap.ui.markerGroup.clearLayers();
	...
	$thematicMap.request = {
		...
		getThemaWomanPeaceFacList : function (base_year, adm_cd) {
			$statsPotal.api.thematicMap.getThemaWomanPeaceFac({
				param : {					
					adm_cd : adm_cd
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successThemaWomanPeaceFac
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				...
				$thematicMap.request.getThemaWomanPeaceFacList($thematicMap.ui.bnd_year, adm_cd);
	...
	$thematicMap.response = {
		...
		successThemaWomanPeaceFac : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successThemaWomanPeaceFac', res.result.peaceInfo);
				$thematicMap.staticJsonObj.push(res.result.peaceInfo);
				console.log('$thematicMap.geoJsonObj', $thematicMap.geoJsonObj);
				$thematicMap.formatter.combinStatsToGeoJon($thematicMap.geoJsonObj, '명', 'pop_100_per_peace_fac_cnt');
				...
				if (poiList.length > 0) {
					for ( var i = 0; i < poiList.length; i++) {
						var _markerIcon = sop.icon({
							iconUrl : '/img/marker/marker/60_03.png',
...
//						html += '</tr>';
					...
					// window.parent.messageAlert.open("알림", "해당 지역 치안시설 검색결과가
					// 없습니다.");
				}
			}
