...
(function (W, D) {
	...
	sMap = {
		...
		map : function () {
			...
			this.createInfoControl = function (val) {
...
//					this._div.innerHTML = '<h4></h4>' + (props ? '<b>' + props.adm_cd + '</b><br />' + adm_nm : '행정동');
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;국민기초생활 수급자수 </th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].basic_liv_recipient) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '</table>';
