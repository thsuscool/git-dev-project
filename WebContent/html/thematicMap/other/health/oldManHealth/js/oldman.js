...
//		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				// getOldManHealthList : function (base_year, adm_cd) {
				$thematicMap.request.getOldManHealthList($thematicMap.ui.bnd_year, adm_cd);
	...
	$thematicMap.response = {
		...
		successOldManHealthList : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successOldManHealthList', res.result.healthfacoldman);
...
//							iconUrl : '/img/marker/marker/10_12.png',
...
//				}
				...
				if (poiList != null && poiList.length > 0) {
					for ( var i = 0; i < poiList.length; i++) {
						var _markerIcon = sop.icon({
							iconUrl : '/img/marker/marker/80_07.png',
							shadowUrl : '/img/marker/theme_shadow.png',
							iconAnchor : [ 13, 40 ],
							iconSize : [ 25, 40 ],
							infoWindowAnchor: [1, -34]
