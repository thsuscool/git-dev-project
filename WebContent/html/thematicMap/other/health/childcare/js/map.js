					...
					this.gMap.on("zoomend", function (e) {
						...
						// 집계구 경계표출
							...
							if (that.curPolygonCode != mapPolygonCode) {
								...
								if (!that.isInnerMapShow) {
									if (that.isDrop) {
										that.openApiBoundaryStatsarea(that.curDropCd);
									}
								}
							}
				...
				if (res.errCd == "0") {
					...
					if (!that.isInnerMapShow) {
						...
						case 1:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == null && that.lastGeojsonInfo.info == "countryArea") {
								...
								that.MainObj.request.AreaAndStatsApis(null);
								console.log('전국');
						...
						// 집계구
						case 5:
							console.log('that.lastGeojsonInfo', that.lastGeojsonInfo);
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd + curDongCd && that.lastGeojsonInfo.year == that.bnd_year) {
								...
								console.log('집계구');
								that.MainObj.request.AreaAndStatsApis(curSidoCd + curSiggCd);
