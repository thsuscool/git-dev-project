...
//				window.parent.$mapNavigation.ui.sidoSelectSet(1); 
		...
		setGeoJson : function (geoJsonObj) {
			...
			//신규통계
			console.log("dataGeojson:"+$thematicMap.ui.sMap.dataGeojson);
	...
	$thematicMap.request = {
		...
		getThemeKosis : function (base_year, adm_cd) {
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				...
				$thematicMap.request.getThemeKosis();
	...
	$thematicMap.response = {
		...
		successThemeKosis : function (status, res) {
			if (res.errCd === 0) {
				...
				addrCdToNm($thematicMap.ui.bnd_year, "00", logOptions);
				
				console.log('successThemeKosis', res.result.kosisInfo);
				$thematicMap.staticJsonObj.push(res.result.kosisInfo);
				console.log('$thematicMap.geoJsonObj', $thematicMap.geoJsonObj);
				$thematicMap.formatter.combinStatsToGeoJon($thematicMap.geoJsonObj, '개', 'lib_pres_condtn');
			}
