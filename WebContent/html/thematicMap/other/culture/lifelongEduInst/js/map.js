		...
		map : function () {
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;평생 교육 기관</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].lifelong_edu_inst) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '</table>';
