		...
		map : function () {
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;인구</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].ppltn) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += ' <th style="text-align: left; word-break:break-all;white-space: nowrap;width:50px;padding-bottom:5px;font-size:12px;">&nbsp;&nbsp;&nbsp;문화시설 1개당 인구</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].pop_100_per_cul_fac_cnt) + '&nbsp;(명/개)';
//					if(data.info[0].cul_fac_cnt < 1) {
//						html += '없음';
//					} else {
//						var tempVal = (data.info[0].ppltn / data.info[0].cul_fac_cnt).toString();
//						if(tempVal.indexOf(".") > 0) {
//							tempVal = tempVal.substring(0, tempVal.indexOf(".") + 2);
//						}
//						html += appendCommaToNumber(tempVal) + '&nbsp;(명/개)';
//					}
					html += '</td>';
					html += '</tr>';
					html += '</table>';
					...
					if (!that.isInnerMapShow) {
						...
						case 1:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == null && that.lastGeojsonInfo.info == "sidoArea") {
								...
								that.MainObj.request.AreaAndStatsApis(null, "2010");
								console.log('전국');
						...
						case 2:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd && that.lastGeojsonInfo.year == "2010") {
								...
								that.MainObj.request.AreaAndStatsApis(curSidoCd, "2010");
								console.log('시군구');
						...
						case 3:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd && that.lastGeojsonInfo.year == "2010") {
								...
								console.log('동면읍');
								that.MainObj.request.AreaAndStatsApis(curSidoCd + curSiggCd, '2010');
								// that.openApiBoundaryHadmarea(curSidoCd +
								// curSiggCd, "2010", "1");
							}
							break;											
