...
//				$thematicMap.ui.markerGroup.clearLayers();
	...
	$thematicMap.request = {
		...
		getThemaPPLTnCultureList : function (base_year, adm_cd) {
			$statsPotal.api.thematicMap.getThemaPPLTnCulture({
				param : {					
					adm_cd : adm_cd
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successgetThemaPPLTnCulture
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				...
				$thematicMap.request.getThemaPPLTnCultureList($thematicMap.ui.bnd_year, adm_cd);
	...
	$thematicMap.response = {
		...
		successgetThemaPPLTnCulture : function (status, res, options) {
			if (res.errCd === 0) {
				...
				console.log('successgetThemaPPLTnCulture', res.result.ppltnculture);
				$thematicMap.staticJsonObj.push(res.result.ppltnculture);
				console.log('$thematicMap.geoJsonObj', $thematicMap.geoJsonObj);
				$thematicMap.formatter.combinStatsToGeoJon($thematicMap.geoJsonObj, '명', 'pop_100_per_cul_fac_cnt');
			}
				...
				if (poiList.length > 0) {
					for ( var i = 0; i < poiList.length; i++) {
						var _markerIcon = sop.icon({
							iconUrl : '/img/marker/marker/90_04.png',
