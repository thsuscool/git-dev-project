...
//				this.sMap.createInfoControl("자가", $thematicMap.leftMap.mapId);
				...
				onEachFeature : function (feature, layer) {
					...
					layer.on({
						...
						mouseout : function(event) {
							$thematicMap.leftMap.sMap.setPolyLayerMouseout(event);
							$thematicMap.rightMap.geoJson.eachLayer(function(layer) {
								if(event.target.feature.properties.adm_cd == layer.feature.properties.adm_cd) {
						...
						sync : function (event) {
							if(event.action == "mouseover") {
								$thematicMap.leftMap.sMap.setPolyLayerMouseover(event);
								$thematicMap.leftMap.sMap.didMouseOverPolygon(event, event.target.feature, "");
	...
	$thematicMap.leftMap.request = {
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.leftMap.setMapzoomLevel(zoom, adm_cd, function () {
				...
				$thematicMap.leftMap.BoundaryApi.getThemaOccupTypeUnder10yearDataList($thematicMap.leftMap, adm_cd);
			});
