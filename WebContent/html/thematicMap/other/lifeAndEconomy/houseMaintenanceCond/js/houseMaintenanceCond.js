...
//				window.parent.$mapNavigation.ui.sidoSelectSet(1); 
		...
		setGeoJson : function (geoJsonObj) {
			...
			var origin = $('#thematicOrigin', parent.document).html();
			...
			$thematicMap.ui.geoJson = sop.geoJson(geoJsonObj, {
				...
				onEachFeature : function (feature, layer) {
					...
					if($thematicMap.ui.isFirst && feature.info.length > 0 && feature.info[0].adm_cd != null && feature.info[0].adm_cd != undefined) {
						if(feature.info[0].adm_cd == currentAdmCd) {
							$thematicMap.ui.isFirst = false;
							$thematicMap.ui.sMap.mapInfo.updateBarChart(feature, origin);
						}
					}
					layer.on({
	...
	$thematicMap.request = {
		...
		getThemaHouseMaintenanceCond : function (adm_cd) {
			$statsPotal.api.thematicMap.getThemaHouseMaintenanceCond({
				param : {
					adm_cd : adm_cd,
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successThemaHouseMaintenanceCond
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				$thematicMap.request.getThemaHouseMaintenanceCond(adm_cd);
			});
	...
	$thematicMap.response = {
		...
		successThemaHouseMaintenanceCond : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successThemaHouseMaintenanceCond', res.result);
				$thematicMap.staticJsonObj.push(res.result.themahousemaintenancecond);
