...
//				$thematicMap.ui.markerGroup.clearLayers();
	...
	$thematicMap.request = {
		...
		getThemaPPLTnChicken : function (base_year, adm_cd) {
			$statsPotal.api.thematicMap.getThemaPPLTnChicken({
				param : {					
					adm_cd : adm_cd
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successThemaPPLTnChicken
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				// getThemaPPLTnChicken : function (base_year, adm_cd) {
				$thematicMap.request.getThemaPPLTnChicken($thematicMap.ui.bnd_year, adm_cd);
	...
	$thematicMap.response = {
		...
		successThemaPPLTnChicken : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successThemaPPLTnChicken', res.result.ppltnchicken);
				$thematicMap.staticJsonObj.push(res.result.ppltnchicken);
				console.log('$thematicMap.geoJsonObj', $thematicMap.geoJsonObj);
				$thematicMap.formatter.combinStatsToGeoJon($thematicMap.geoJsonObj, '개', 'pop_100_per_chicken_corp_cnt');
				...
				if (poiList.length > 0) {
					for ( var i = 0; i < poiList.length; i++) {
						var _markerIcon = sop.icon({
							iconUrl : '/img/marker/marker/50_08.png',
