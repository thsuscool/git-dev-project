		...
		map : function () {
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;치킨집수 </th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].chicken_corp_cnt) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += ' <th style="text-align: left; word-break:break-all;white-space: nowrap;width:50px;padding-bottom:5px;font-size:12px;">&nbsp;&nbsp;&nbsp;치킨집 1개당 인구</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].pop_100_per_chicken_corp_cnt) + '&nbsp;(명/개)';
//					if(data.info[0].chicken_corp_cnt < 1) {
//						html += '없음';
//					} else {
//						var tempVal = (data.info[0].ppltn / data.info[0].chicken_corp_cnt).toString();
