...
//			$($thematicMap.ui.sMap.mapInfo.barChartObj).hide();
	...
	$thematicMap.request = {
		...
		getYoungAdult : function (adm_cd) {
			$statsPotal.api.thematicMap.getYoungAdult({
				param : {
					adm_cd : adm_cd,
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successYoungAdult
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				$thematicMap.request.getYoungAdult(adm_cd);
			});
	...
	$thematicMap.response = {
		...
		successYoungAdult : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successOldManHealthList', res.result);
				$thematicMap.staticJsonObj.push(res.result.youngadult);
