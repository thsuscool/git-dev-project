...
//														"</tr>" +
			...
			this.updateBarChart = function (res) {
				...
				retDataList.push(res.info[0].mlpxhou_cnt_2000);
				retDataList.push(res.info[0].mlpxhou_cnt_2005);
				retDataList.push(res.info[0].mlpxhou_cnt_2010);
				
				var titleStr = "<h3><b>연립/다세대주택수 증감현황 (호)</b></h3> </br>";
				...
				$("#barChart_"+that.id).find("#container_graph_bar").highcharts({
					...
					tooltip : {
						...
						pointFormat : '<tr><td style="color:{series.color};padding:0"></td>' + '<td style="padding:0"><b>{point.y} 호</b></td></tr>',
						footerFormat : '</table>',
						shared : true,
						useHTML : true
					},
