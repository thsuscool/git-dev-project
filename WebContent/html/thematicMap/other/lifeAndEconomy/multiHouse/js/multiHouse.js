...
//			$($thematicMap.ui.sMap.mapInfo.barChartObj).hide();
		...
		setGeoJson : function (geoJsonObj) {
			...
			$thematicMap.ui.geoJson = sop.geoJson(geoJsonObj, {
				...
				onEachFeature : function (feature, layer) {
					...
					if($thematicMap.ui.isFirst && feature.info.lengt && feature.info[0].adm_cd != null && feature.info[0].adm_cd != undefined) {
	...
	$thematicMap.request = {
		...
		GetThemaMultiHouse : function (adm_cd) {
			$statsPotal.api.thematicMap.getThemaMultiHouse({
				param : {
					adm_cd : adm_cd,
				},
				method : 'POST',
				...
				success : $thematicMap.response.successMultiHouse
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				$thematicMap.request.GetThemaMultiHouse(adm_cd);
			});
	...
	$thematicMap.response = {
		...
		successMultiHouse : function (status, res, options) {
			if (res.errCd === 0) {
				...
				console.log('successMultiHouse', res.result);
				$thematicMap.staticJsonObj.push(res.result.multihouse);
