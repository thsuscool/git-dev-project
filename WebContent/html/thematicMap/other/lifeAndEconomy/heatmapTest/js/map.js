		...
		map : function () {
			...
			this.didMouseOverPolygon = function(event, data, type) {
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;연 평균 증감율</th>';
					html += '<td style="text-align:center;">';
			...
			this.addControlEvent = function (type, opt) {
				...
				// 줌 종료
					...
					this.gMap.on("zoomend", function (e) {
						...
						// 시구군단위 경계표출
						else if (that.zoom > 5 && that.zoom <= 7) {
							mapPolygonCode = 2;
						...
						// 동단위 경계표출
						else if (that.zoom > 7 && that.zoom<=9) {
							mapPolygonCode = 3;
