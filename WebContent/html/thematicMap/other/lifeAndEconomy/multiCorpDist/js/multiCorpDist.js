...
//				$thematicMap.ui.markerGroup.clearLayers();
	...
	$thematicMap.request = {
		...
		getThemaMultiCorpDist : function (base_year, adm_cd) {
			$statsPotal.api.thematicMap.getThemaMultiCorpDist({
				param : {					
					adm_cd : adm_cd
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successThemaMultiCorpDist
			});
		},
		...
		getPoiInfobyThemecd : function (_year, _area_type, _adm_cd, _bnd_year, _pagenum, callbackApi) {
			...
			var ksic5List = [];
			ksic5List.push("47511");
			ksic5List.push("47513");
			ksic5List.push("47519");
			ksic5List.push("25111");
			ksic5List.push("42420");
			ksic5List.push("42491");
			ksic5List.push("42201");
			...
			$statsPotal.api.thematicMap.getThemaMultiCorpDistPOI({
				param : param,
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				$thematicMap.request.getThemaMultiCorpDist($thematicMap.ui.bnd_year, adm_cd);
				
				if ($thematicMap.ui.map.getZoom() > 8)
					$thematicMap.request.getPoiInfobyThemecd($thematicMap.ui.bnd_year, '0', adm_cd, $thematicMap.ui.bnd_year, '0');
			});
	...
	$thematicMap.response = {
		...
		successThemaMultiCorpDist : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successThemaMultiCorpDist', res.result.themamulticorpdist);
				$thematicMap.staticJsonObj.push(res.result.themamulticorpdist);
				console.log('$thematicMap.geoJsonObj', $thematicMap.geoJsonObj);
				$thematicMap.formatter.combinStatsToGeoJon($thematicMap.geoJsonObj, '개', 'house_10_corp_per');
				...
				addrCdToNm($thematicMap.ui.bnd_year, options.params.adm_cd, logOptions);
			}
		...
		successPoiInfobyThemecd : function (status, res, options) {
			if (res.errCd == '-401') {
				...
				var poiList = res.result.themamulticorpdistPOI;
