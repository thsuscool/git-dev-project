		...
		map : function () {
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;주택 유지보수 업체수 </th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].biz_cnt) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += ' <th style="text-align: left; word-break:break-all;white-space: nowrap;width:50px;padding-bottom:5px;font-size:12px;">&nbsp;&nbsp;&nbsp;업체 1개당 주택수</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].house_10_corp_per) + '&nbsp;(호/개)';
//					if(data.info[0].corp_no < 1) {
//						html += '없음';
//					} else {
//						var tempVal = (data.info[0].house_no / data.info[0].corp_no).toString();
//						if(tempVal.indexOf(".") > 0) {
//							tempVal = tempVal.substring(0, tempVal.indexOf(".") + 2);
//						}
//						html += appendCommaToNumber(tempVal) + '&nbsp;(호/개)';
//					}
					html += '</td>';
					html += '</tr>';
					html += '</table>';
