...
//				window.parent.$mapNavigation.ui.sidoSelectSet(1); 
				...
				/*신규범례추가*/ 
				var legend = new sLegendInfo.legendInfo($thematicMap.ui.sMap);			
				legend.initialize($thematicMap.ui);
				$thematicMap.ui.sMap.legend = legend;
				legend.createLegend();
...
//			$($thematicMap.ui.sMap.mapInfo.barChartObj).hide();
	...
	$thematicMap.request = {
		...
		getOldPPLTn : function (adm_cd) {
			$statsPotal.api.thematicMap.getOldPPLTn({
				param : {
					adm_cd : adm_cd,
				},
				options : {
				...
				method : 'POST',
				success : $thematicMap.response.successOldPPLTn
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				...
				$thematicMap.request.getOldPPLTn(adm_cd);
				
				// getPoiInfobyThemecd : function (_year, _area_type, _adm_cd,
				// _bnd_year) {
//				if ($thematicMap.ui.map.getZoom() > 8)
//					$thematicMap.request.getPoiInfobyThemecd('2012', '0', adm_cd, '2012', '0');
			});
	...
	$thematicMap.response = {
		...
		successOldPPLTn : function (status, res, options) {
			if (res.errCd === 0) {
				...
				addrCdToNm("2010", options.params.adm_cd, logOptions);
				
				console.log('successOldManHealthList', res.result);
				$thematicMap.staticJsonObj.push(res.result.oldppltn);
		...
		successPoiInfobyThemecd : function (status, res, options) {
			if (res.errCd == '-401') {
				...
				setTimeout(function () {
					...
					// _adm_cd, _bnd_year) {
					$thematicMap.request.getPoiInfobyThemecd(options.param.year, options.param.area_type, options.param.adm_cd, options.param.bnd_year, options.callbackApi);
				}, 500);
			} else {
				console.log('res', res);
				...
				var apicallCount = parseInt(totalCount / (returnCount * pageNum));
				
				console.log('apicallCount',apicallCount);
				if (apicallCount > 0) {					
						$thematicMap.request.getPoiInfobyThemecd('2012', '0', options.adm_cd, '2012', pageNum+1);					
				}
				...
				if (poiList.length > 0) {
					for ( var i = 0; i < poiList.length; i++) {
						...
						html += '<tr>';
						html += '<th style="word-break:break-all;width:30%;padding:5px;">이름 </th>';
						html += '<td >';
						html += poiList[i].corp_nm;
						html += '</td>';
						html += '</tr>';
						html += '<tr>';
						html += ' <th style="word-break:break-all;white-space: nowrap;width:50px;padding:5px;">주소 </th>';
						html += '<td >';
						html += poiList[i].nddr;
						html += '</td>';
						html += '</tr>';
						html += '<tr>';
						html += ' <th style="word-break:break-all;white-space: nowrap;width:50px;padding:5px;">전화번호 </th>';
						html += '<td >';
						html += poiList[i].tel_no;
						html += '</td>';
						html += '</tr>';
						html += '</table>';
					...
					alert('해당 지역 보건업종 검색결과가 없습니다.');
					messageAlert.open("알림", "해당 지역 보건업종 검색결과가 없습니다.");
					messageAlert.open('', 'test');
				}
			}
	...
	$thematicMap.formatter = {
		setLegendForStatsData : function (arData) {
			...
			//신규범례추가
			$thematicMap.ui.sMap.legend.valPerSlice = $thematicMap.ui.sMap.legend.calculateLegend(arData);
		},
