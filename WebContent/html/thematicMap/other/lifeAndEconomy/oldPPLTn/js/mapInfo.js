	...
	sMapInfo = {
		mapInfo : function (map) {
			...
			this.createNavi = function () {
				var infoNavi = sop.control({
					position : 'topleft'
				});
				infoNavi.onAdd = function (map) {
					this._div = sop.DomUtil.create('div', 'info');
					sop.DomEvent.disableClickPropagation(this._div);
					this.update();
					$(this._div).attr("id", 'navi_' + that.id);
					return this._div;
				};
				infoNavi.update = function (props) {
					this._div.innerHTML = "<div>" + "<select id='sidoSelect' class='navi_select_background' ></select>" + "<select id='sggSelect' class='navi_select_background'></select>" + "<select id='admSelect' class='navi_select_background'><option value=''>동/읍면 선택</option></select>" + "</div>";
				};
				infoNavi.addTo(this.map.gMap);
				
				// 전국선택
				$("#navi_" + this.id).find("#contrySelect").click(function () {
					that.allCountrySet()
				});
				
				// 시도선택
				$("#navi_" + this.id).find("#sidoSelect").change(function () {
					that.sggSelectSet(this.value);
				});
				
				// 시군구선택
				$("#navi_" + this.id).find("#sggSelect").change(function () {
					that.admSelectSet(this.value);
				});
				
				// 읍면동선택
				$("#navi_" + this.id).find("#admSelect").change(function () {
					that.admOnchange(this.value);
				});
					...
					this._div.innerHTML = "	<div class='legend_remark_section'>" +
														...
														"<div class='remarkbox'>" +
															"<div class='remarkbox_in'>" +
																...
																"<a class='btn_remark_offset'><img class='rollover' src='/img/im/icon_arrow_remark_off.gif' alt='' /></a>" +
															"</div>" +
														"</div>" +
													"</div>";
				};
...
//														"</tr>" +
														"<tr>" +
															...
															"<td>" +
																"<ul>" +
																	...
																	"<li>" +
																		"<div class='mixcolorbox'>" +
																			"<input id='radioRedColor' name='setColor_"+that.id+"' type='radio' value='Red' checked='checked' />" +
																			"<span class='color_03'>" +
														...
														"<tr>" +
															...
															"<td>" +
																...
																"<input type='radio' name='selectEvenLegend_"+that.id+"' id='legend_equal' value='equal' /> 균등범례" +
															"</td>" +
														"</tr>" +
													"</tbody>" +
												"</table>" +
				...
				infoBarChart.update = function (props) {
					this._div.innerHTML = 	"<div class='graph_bar_remark_section'>" +
													...
													"<div class='graphbox_bar'>" +
														...
														"<p id='bar_graph_title' class='graph_txt'></p>" +
															"<div class='graph'>" +
																"<div id='container_graph_bar'>" +
																"</div>" +
															"</div>" +
														"</div>" +
														"<a class='btn_graph_bar_offset'><img class='rollover' src='/img/im/icon_arrow_graph_off.gif' alt='' /></a>" +
														"<div id='graphbox_bar_append'></div>" +
													"</div>" +
												"</div>";
				};
			...
			this.updateBarChart = function (res) {
				...
				retDataList.push(res.info[0].aged_psn_ppltn_cnt_2000);
				retDataList.push(res.info[0].aged_psn_ppltn_cnt_2005);
				retDataList.push(res.info[0].aged_psn_ppltn_cnt_2010);
				
				var titleStr = "<h3><b>65세 이상 고령자 증감현황 (명)</b></h3> </br>";
				titleStr += res.properties.adm_nm;
				$("#bar_graph_title").html(titleStr);
				...
				$("#barChart_"+that.id).find("#container_graph_bar").highcharts({
					...
					tooltip : {
						...
						pointFormat : '<tr><td style="color:{series.color};padding:0"></td>' + '<td style="padding:0"><b>{point.y} 명</b></td></tr>',
						footerFormat : '</table>',
						shared : true,
						useHTML : true
					},
				...
				html += "출처 : 통계청, 2000,2005,2010년 인구주택총조사</br>";
				html += "</div>";
				html += "</br>";
//				$(this.barChartObj).append(html);
				$("#barChart_"+that.id).find("#graphbox_bar_append").html(html);
			};
