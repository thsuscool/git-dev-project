...
//						var html = '<table style="text-align:left;width:250px;white-space: nowrap;word-break:break-all;padding:5px;" >';
//						html += '<tr>';
//						html += '<th style="text-align: left; word-break:break-all;padding:5px;font-size:14px;"><strong></strong></th>';
//						html += '</tr>';
//						html += '</table>';
//						this._div.innerHTML = html;
				...
				$(".leftWindow").css("padding", "6px 8px").css("font", "12px/14px Arial, Helvetica, sans-serif").css("background", "white").css("background", "rgba(255,255,255,1)").css("box-shadow", "0 0 15px rgba(0,0,0,0.2)").css("border-radius", "5px");
			},
			
//			this.createLeftControl = function (year) {
...
//						}
//					}
//					this._div.innerHTML = '<h4>' + year + '</h4>';
//					
//				};
//				this.infoControl = infoControl;
//				infoControl.addTo(this.gMap);
...
//					console.log(data);
//					console.log(that.thisObj.mapId);
					
					if (that.thisObj.mapId == 'leftMap') {
						showdata = data.info[0].family_cnt;
						showText = "다문화 가구수";
					}
					else {
						showdata = data.info[0].household_cnt;
						showText = "총 가구수";
					}
					var html = '<table style="text-align:left;width:250px;white-space: nowrap;word-break:break-all;padding:5px;" >';
					html += '<tr>';
					...
					html += appendCommaToNumber(showdata) + '&nbsp;' + data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '</table>';
...
//						pane : 'infowindowPane'
//					
//					}).addTo(that.thisObj.map)._showToolTip(event);
//					event.target.tooltip.updateZIndex(900000);
					
					that.thisObj.sMap.mapInfo.selectChartData(data.properties, data.dataIdx);
					
					/* 신규범례 추가  */
					that.legend.selectLegendRangeData(event.target.options.fillColor);
				}
				else {
					var html = '<table style="text-align:left;width:250px;white-space: nowrap;word-break:break-all;padding:5px;" >';
					html += '<tr>';
					html += '<th style="word-break:break-all;padding:5px;font-size:14px;"><strong>' + data.properties.adm_nm + '</strong></th>';
					html += '<td style="text-align:center;">&nbsp;';
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += '<th style="word-break:break-all;padding-bottom:5px;font-size:12px;">&nbsp;데이터가 없습니다.</th>';
					html += '<td style="text-align:center;">';
					html += '</td>';
					html += '</tr>';
					html += '</table>';
...
//			this.didMouseOverPolygon = function (event, data, type) {		
//				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
//					var html = '<table style="text-align:left;width:auto;min-width:200px;white-space: nowrap;word-break:break-all;padding:5px;" >';
//					html += '<tr>';
//					if(data.properties.adm_nm != "null") {
//						html += '<th style="text-align: left; word-break:break-all;width:30%;padding:5px;font-size:14px;"><strong>'+data.properties.adm_nm+'</strong></th>';
//					} else {
//						html += '<th style="text-align: left; word-break:break-all;width:30%;padding:5px;font-size:14px;"><strong>집계구</strong></th>';
//					}
//					html += '<td style="text-align:center;">';					
//					html += '</td>';
//					html += '</tr>';
//					html += '<tr>';
//					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;다문화 가구 수 </th>';
//					html += '<td style="text-align:right;">';
//					html += appendCommaToNumber(data.info[0].family_cnt) + '&nbsp;'+data.info[0].unit;
//					html += '</td>';
//					html += '</tr>';
//					html += '</table>';
//					event.target.bindToolTip(html, {
...
//					html += '<tr>';
//					html += '<th style="word-break:break-all;width:30%;padding:5px;font-size:14px;"><strong>'+data.properties.adm_nm+'</strong></th>';
//					html += '<td style="text-align:center;">&nbsp;';					
//					html += '</td>';
//					html += '</tr>';
//					html += '<tr>';
//					html += '<th style="word-break:break-all;width:30%;padding-bottom:5px;font-size:12px;">&nbsp;데이터가 없습니다.</th>';
//					html += '<td style="text-align:center;">';					
//					html += '</td>';
//					html += '</tr>';
//					html += '</table>';
...
//					this.gMap.on("moveend", function (e) {
					...
					this.gMap.on("zoomend", function (e) {
						...
						// 동단위 경계표출
							...
							if (that.curPolygonCode != mapPolygonCode) {
								...
								if (!that.isInnerMapShow) {
									if (that.isDrop) {
										that.openApiBoundaryHadmarea(that.curDropCd, "2010", "1");
									}
								}
							}
							that.curPolygonCode = mapPolygonCode;
			...
			// OpenAPI 리버스지오코딩
			this.openApiReverseGeoCode = function (center, bnd_year) {
				...
				sopOpenApiReverseGeoCodeObj.addParam("bnd_year", bnd_year);
...
//									that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
									...
									console.log(that.thisObj.mapId, 'aleady exist');
									// that.clearData();
								}
							}
							else {
								that.thisObj.request.AreaAndStatsApis(null, "2010", target);
								console.log(that.thisObj.mapId, '전국');
						...
						case 2:
							// 최근경계조회조건 하나를 가지고 있다가 비교하여 같으면 기존에 저장된 경계를 호출
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd && that.lastGeojsonInfo.year == "2010") {
								that.MainObj.Popup.close();
								if (that.dataGeojson == null) {
//									that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
								}
								...
								that.thisObj.request.AreaAndStatsApis(curSidoCd, "2010", target);
								console.log(that.thisObj.mapId, '시군구');
						...
						// 동면읍
						case 3:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd && that.lastGeojsonInfo.year == "2010") {
								that.MainObj.Popup.close();
								if (that.dataGeojson == null) {
//									that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
								...
								// Api 요청
//								console.log('that.MainObj', that.MainObj);
//								console.log('동면읍');
								that.thisObj.request.AreaAndStatsApis(curSidoCd + curSiggCd, '2010', target);
								// that.openApiBoundaryHadmarea(curSidoCd +
								// curSiggCd, "2010", "1");
							}
							break;
						...
						// 단일동
						case 4:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd + curDongCd && that.lastGeojsonInfo.year == "2010") {
								that.MainObj.Popup.close();
								if (that.dataGeojson == null) {
//									that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
								...
								that.thisObj.request.AreaAndStatsApis(curSidoCd + curSiggCd + curDongCd, '2010', target);
...
//						that.thisObj.mapNavi2.reverseOnSelectChange(that);
//						window.parent.$mapNavigation_2.ui.reverseOnSelectChange(that);
					}
									
				}
				else if (res.errCd == "-401") {
					accessTokenInfo();
					setTimeout(that.openApiReverseGeoCode(options.center, options.bnd_year), 500);
				}
				else {
