...
//				window.parent.$mapNavigation.ui.sidoSelectSet(1); 
	...
	$thematicMap.request = {
		...
		getThemaSmallBizCond : function (adm_cd) {
			$statsPotal.api.thematicMap.getThemaSmallBizCond({
				param : {
					adm_cd : adm_cd,
					corp_cd : $thematicMap.ui.class_cd,
				},
				options : {
					params : {
						adm_cd : adm_cd,
				...
				method : 'POST',
				success : $thematicMap.response.successThemaSmallBizCond
			});
		},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				$thematicMap.request.getThemaSmallBizCond(adm_cd);
			});
	...
	$thematicMap.response = {
		...
		successThemaSmallBizCond : function (status, res, options) {
			if (res.errCd === 0) {
				console.log('successThemaSmallBizCond', res.result);
				$thematicMap.staticJsonObj.push(res.result.themasmallbizcond);
