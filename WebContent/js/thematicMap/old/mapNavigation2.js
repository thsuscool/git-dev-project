 ...
 */
(function (W, D) {
	W.$mapNavigation_2 = W.$mapNavigation_2 || {};
	...
	$mapNavigation_2.ui = {		
		...
		sidoSelectSet : function (type) {
			...
			var sopPortalSidoObj = new sop.portal.sido2.api();
		...
		sggSelectSet : function (type, sido_cd) {
			...
			var sopPortalSggObj = new sop.portal.sgg2.api();
		...
		admSelectSet : function (type, sgg_cd) {
			...
			if (sgg_cd == "") {
				...
				var sopPortalAdmObj = new sop.portal.adm2.api();
		...
		reverseOnSelectChange : function () {
			...
			var sopPortalAllAddressObj = new sop.portal.allAddress2.api();
	...
	$mapNavigation_2.event = {
		...
		setUIEvent : function () {
			...
			$("#contrySelect_2").click(function () {
				$mapNavigation_2.ui.allCountrySet(2);
			});
					
			//시도선택
			$("#sidoSelect_2").change(function () {
				$mapNavigation_2.ui.sggSelectSet(2, this.value);
			});
			
						
			//시군구선택
			$("#sggSelect_2").change(function () {
				$mapNavigation_2.ui.admSelectSet(2, this.value);
			});
						
			//읍면동선택
			$("#admSelect_2").change(function () {
				$mapNavigation_2.ui.admOnchange(2, this.value);
			});
		}
	};
	...
	/*********** 시도 검색 Start **********/
	(function () {
		$class("sop.portal.sido2.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				...
				if (res.errCd == "0") {
					...
					$mapNavigation_2.ui.sggSelectSet(type, $("#sidoSelect_" + type).val());
				}
				else {
	...
	/*********** 시군구 검색 Start **********/
	(function () {
		$class("sop.portal.sgg2.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				var result = res.result;
				var type = options.type;
				if (res.errCd == "0") {
					...
					for ( var i = 0; i < result.sggList.length; i++) {
						...
						//첫화면은 구단위
						if ($mapNavigation_2.ui.firstBoolean == true && elem.sgg_cd == "240") {
							$mapNavigation_2.ui.firstValue = elem.sgg_cd + "/" + elem.x_coor + "/" + elem.y_coor;
							$mapNavigation_2.ui.firstSggCd = elem.sgg_cd;
						}
					}
					...
					//첫화면은 구단위
					if ($mapNavigation_2.ui.firstBoolean) {
						$("#sggSelect_" + type).val($mapNavigation_2.ui.firstValue).attr("selected", "selected");
						$mapNavigation_2.ui.admSelectSet(type, $mapNavigation_2.ui.firstValue);
					}
				}
				else {
					messageAlert.open("알림", res.errMsg);
	...
	/*********** 읍면동 검색 Start **********/
	(function () {
		$class("sop.portal.adm2.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				var result = res.result;
				var type = options.type;
				if (res.errCd == "0") {
					...
					for ( var i = 0; i < result.admList.length; i++) {
						...
						//첫화면은 특정지역
						if ($mapNavigation_2.ui.firstBoolean == true && elem.emdong_cd == "65") {
							//$mapNavigation_2.ui.firstValue  = elem.emdong_cd + "/" + elem.x_coor + "/" + elem.y_coor;
							//$mapNavigation_2.ui.firstAdmCd = elem.emdong_cd;
							//x_coor = elem.x_coor;
							//y_coor = elem.y_coor;
						}
					}
					...
					//첫화면은 특정지역
					if ($mapNavigation_2.ui.firstBoolean) {
						$mapNavigation_2.ui.firstBoolean = false;
						//$("#admSelect_"+type).val($mapNavigation_2.ui.firstValue).attr("selected", "selected");
						//지도 위치 이동
						//$mapNavigation_2.ui.curMap.mapMove([x_coor, y_coor] , 9);
						
						//오픈API 호출
						//	var sidoArray = $("#sidoSelect_"+type).val().split("/");
						//	$mapNavigation_2.ui.curMap.curSidoCd = sidoArray[0];
						//	$mapNavigation_2.ui.curMap.curSiggCd = $mapNavigation_2.ui.firstSggCd;
						//	$mapNavigation_2.ui.curMap.curDongCd = $mapNavigation_2.ui.firstAdmCd;
					}
				}
				else {
					messageAlert.open("알림", res.errMsg);
	...
	(function () {
		$class("sop.portal.allAddress2.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				var result = res.result;
				var type = options.type;
				if (res.errCd == "0") {
					...
					$mapNavigation_2.ui.curMap.curSidoCd = result.sido_cd;
					$mapNavigation_2.ui.curMap.curSiggCd = result.sgg_cd;
					$mapNavigation_2.ui.curMap.curDongCd = result.dong_cd;
