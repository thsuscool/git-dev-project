...
(function(W, D) {
	...
	$(window).load(function() {
		setTimeout(function() {
			if (window.opener.$thematicMapFrame07 != undefined) {
				window.opener.$thematicMapFrame07.ui.reportLoad();
			}
		}, 500);
	});
...
//					this.setGrid(data, options,dataGeoJson);
				...
				setMainReportInfo : function(data) {
					...
					$("#date").html(date);
					$("#origin").html(data.origin); //2017.03.09 보고서 수정
				...
				setLegend : function(data, options) {
					...
					if (window.opener.$thematicMapFrame07 != undefined) {
						map = window.opener.$thematicMapFrame07.ui.mapList[data.id];
					}
				...
				drawGrid : function(data) {
					...
					for (var i=0; i<gridList.length; i++) {
						html += "<tr>";
		 				...
		 				html += 	"<td>"+gridList[i].worker_sum+"</td>"; //2017.03.08 보고서 수정
		 				html += "</tr>";
					}
