...
(function(W, D) {
	...
	$(window).load(function() {
		setTimeout(function() {
			if (window.opener.$thematicMapFrame05 != undefined) {
				window.opener.$thematicMapFrame05.ui.reportLoad();
			}
		}, 500);
	});
	...
	$reportForm.ui = {
				...
				setMainReportInfo : function(data) {
					...
					if (this.mapType == "thematicMap") {
						mainTitle = data.param.title+" "+"("+data.base_year+")";
				...
				setLegend : function(data, options) {
					...
					if (window.opener.$thematicMapFrame05 != undefined) {
						map = window.opener.$thematicMapFrame05.ui.mapList[data.id];
					}
...
//					if (this.mapType == "interactiveMap") {
					if (this.mapType == "thematicMap") {
						if (data.param.isKosis != undefined && data.param.isKosis) {
							...
							} else if(data.selectOption=="rightValue"){
								unit = data.param.right_sep_unit;
							} else {
								unit = data.param.sep_map_left_sep_unit;
							}
				...
				setGrid : function(data, options,dataGeoJson) {
					...
					if (this.mapType == "thematicMap") {
						...
						if(data.selectOption=="leftValue"){
							...
							unit = data.param.sep_map_left_sep_ttip_title+"("+data.param.sep_map_left_sep_unit+")";
