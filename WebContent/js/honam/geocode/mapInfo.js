...
//																	"</tr>" +
				...
				this.geocode = function(tmpBody, idx){
					setTimeout(function() {
						if (idx < tmpBody.length) {
							var addr = tmpBody[idx].addr;
							$('#userUpload_'+that.id+' tbody > tr').eq(idx).find("td").eq(0).css("background-color", "#fffa82");
							var index = that.dataTable.column("status:name").index();
							that.dataTable.cell(idx, index).data("전송시작");

							var geocoder = new daum.maps.services.Geocoder();
							geocoder.addr2coord(addr, function(status, result) {
							     if (status === daum.maps.services.Status.OK) {
							        var coords = new sop.LatLng(result.addr[0].lat, result.addr[0].lng);
					            	var xIdx = that.dataTable.column("x:name").index();
				            		var yIdx = that.dataTable.column("y:name").index();
				            		var statusIdx = that.dataTable.column("status:name").index();

				            		that.dataTable.cell(idx, xIdx).data(parseInt(coords.x)+"");
				            		that.dataTable.cell(idx, yIdx).data(parseInt(coords.y)+"");
				            		
				            		that.dataTable.cell(idx, statusIdx).data("변환성공");
				            		$('#userUpload_'+that.id+' tbody > tr').eq(idx).find("td").eq(0).css("background-color", "#b2fa5c");
				            		
							        idx++;
							        that.geocode(tmpBody, idx);
							     }else {
							    	var statusIdx = that.dataTable.column("status:name").index();
							    	that.dataTable.cell(idx, statusIdx).data("변환실패");
					            	$('#userUpload_'+that.id+' tbody > tr').eq(idx).find("td").eq(0).css("background-color", "#ff9999"); 
