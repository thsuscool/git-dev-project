...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		themaModify.receiveType = getParameter("post_no");
		themaModify.receiveCategoryCd = getParameter("board_cd");
		themaModify.makeThemaLists();
		//themaModify.readyRegistThema();
	})
	...
	themaModify = {
		...
		// 게시판 목록 페이징 처리
		themaModifyPaging : function (totalCount, currentIndex) {
			...
			$('.pagenation2 .pages').paging({
				...
				onclick : function(e,page){							// 페이지 선택 시
					themaModify.currentPageIndex = page
					themaModify.makeThemaLists();
				}
			});
		},
		...
		makeThemaLists : function () {
			themaModify.removeFileOnModifyMode();
			...
			var sopPortalThemaObj = new sop.portal.themaModifyList.api();
			sopPortalThemaObj.addParam("board_cd", themaModify.receiveCategoryCd);
			sopPortalThemaObj.addParam("post_no", themaModify.receiveType);
			//sopPortalThemaObj.addParam("low_rank_s_class_cd", themaModify.low_rank_s_class_cd);
			if(themaModify.searchWord != null && themaModify.searchWord.length > 0) {
				sopPortalThemaObj.addParam(themaModify.searchType, themaModify.searchWord.toUpperCase());
			}
		...
		getThemaListsReply : function () {
			var sopPortalThemaObj = new sop.portal.themaModifyReply.api();
			sopPortalThemaObj.addParam("board_cd", themaModify.receiveLists[themaModify.selectIndex].board_cd);
			sopPortalThemaObj.addParam("post_no", themaModify.receiveLists[themaModify.selectIndex].post_no);
			sopPortalThemaObj.request({
				method : "POST",
				async : true,
		...
		makeModifyAndDeleteBtn : function (index) {
			if(index == themaModify.selectIndex) {
				return;
			}

			if(themaModify.prevHtmlSouce != null) {
				$("#list_" + themaModify.selectIndex).html(themaModify.prevHtmlSouce);
			}

			themaModify.selectIndex = index;
			themaModify.prevHtmlSouce = $("#list_" + themaModify.selectIndex).html();
		...
		/*setThemaData : function () {
			...
			themaModify.isListNotClose = true;
			...
			themaModify.isRegistMode = false;
			
			var selectObj = themaModify.receiveLists[themaModify.selectIndex];
		...
		themaModify : function () {
			...
			if(file != null && file.length > 1 && file.trim().length > 1) {
				...
				/*sopPortalThemaObj.addParam("board_cd", themaModify.receiveLists[0].board_cd);*/
				sopPortalThemaObj.addParam("post_no", themaModify.receiveType);
...
//					console.log("beforeSend");
				...
				success: function(data) {
					if(data.errCd == "0") {
						...
						themaModify.isRegistMode = true;
						themaModify.selectIndex = null;
						themaModify.prevHtmlSouce = null;
						board.refreshSecretCode();
						themaModify.makeThemaLists();
...
//		        	console.log("complete");
		...
		setThemaData : function () {
			...
			window.location.href = contextPath + "/view/board/qnaThemaModify?post_no="+themaModify.receiveType+"&board_cd="+themaModify.receiveCategoryCd;
		...
		downloadFile : function(index) {
	        ...
	        target.attr('value', themaModify.receiveLists[themaModify.selectIndex].file_id);

	        target = $('#file_nm');
	        target.attr('name', 'file_nm');
	        target.attr('value', themaModify.receiveLists[themaModify.selectIndex].file_nm);
	        
	        target = $('#file_path');
	        target.attr('name', 'file_path');
	        target.attr('value', themaModify.receiveLists[themaModify.selectIndex].file_path);

	        target = $('#file_extension');
	        target.attr('name', 'file_extension');
	        target.attr('value', themaModify.receiveLists[themaModify.selectIndex].file_extension);
	        
	        target = $('#file_content_type');
	        target.attr('name', 'file_content_type');
	        target.attr('value', themaModify.receiveLists[themaModify.selectIndex].file_content_type);

	        $('#tempForm').submit();
	        $('#tempForm').remove();
	        
//			var sopPortalThemaObj = new sop.portal.themaGetSecretCodeCorrect.api();
//			sopPortalThemaObj.addParam("file_id", themaModify.receiveLists[themaModify.selectIndex].file_id);
//			sopPortalThemaObj.addParam("file_nm", themaModify.receiveLists[themaModify.selectIndex].file_nm);
//			sopPortalThemaObj.addParam("file_path", themaModify.receiveLists[themaModify.selectIndex].file_path);
//			sopPortalThemaObj.addParam("file_extension", themaModify.receiveLists[themaModify.selectIndex].file_extension);
//			sopPortalThemaObj.request({
//				method : "POST",
//				async : false,
...
//			});
		...
		controlListItem : function(post_no) {
			if(themaModify.isListNotClose) {
				themaModify.isListNotClose = false;
				return;
			}
			window.location.href = contextPath + "/view/board/qnathemaModify?post_no="+post_no+"&board_cd="+themaModify.themaCd;
			/*if(index < 0) {
			...
			if ( fl == "on" ){
				...
				themaModify.modifyPostHits(themaModify.receiveLists[index])
	...
	/*********** Receive Lists Start **********/
	(function() {
		$class("sop.portal.themaModifyList.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res) {
				if(res.errCd == "0") {
					var result = res.result;
					console.log(result);
					$("#thema_title_text").html("&nbsp;&nbsp;&nbsp;총" + result.total_count + " 건");
					themaModify.themaModifyPaging(result.total_count, themaModify.currentPageIndex);

					themaModify.receiveLists = result.summaryList;
					...
					for(var i = 0; i < result.summaryList.length; i ++) {
						...
						$("#thema_regist_title_input").val(title);
						$("#thema_regist_content_input").val(content);
						
						if(listItem.file_nm !== null && listItem.file_nm !== undefined) {
							$("#thema_file").prop('disabled', true);
							$("#themaFileTrName").html(listItem.file_nm + "." + listItem.file_extension);
							$("#themaFileTr").show();
						}
					}
					
					var selectObj = themaModify.receiveLists[0];
...
//					}
					themaModify.isListNotClose = false;
					//themaModify.controlListItem(-1);
				} else {
					messageAlert.open("알림", res.errMsg);
				}
				
				if (themaModify.receiveType != undefined && themaModify.receiveType != null && themaModify.receiveType.length > 0) {
					if(themaModify.receiveType == 'thema') {
						themaModify.setTab();
						themaModify.receiveType = null;
					}
				}
				
//				if (themaModify.receiveCategoryCd != undefined && themaModify.receiveCategoryCd != null && themaModify.receiveCategoryCd.length > 0) {
//					$("#thema_regist_category_select").combobox('select', board.lowRankSClassCd[0].value);
//					for(var i = 0; i < board.lowRankSClassCd.length; i++) {
//						if(board.lowRankSClassCd[i].value == themaModify.receiveCategoryCd) {
//							$("#thema_regist_category_select").combobox('select', themaModify.receiveCategoryCd);
//							return;
//						}
//					}
//				}
			...
			onSuccess : function(status, res) {
				if(res.errCd == "0") {
					...
					for(var i = 0; i < result.summaryList.length; i ++) {
						...
						html += "<tr>";
						html += "<td id='list_" + i + "' ";
						html += "onclick='javascript:themaModify.makeModifyAndDeleteBtn(" + i + ")' style='cursor:pointer;'>";
	...
	(function() {
		$class("sop.portal.themaModifyReply.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res) {
				if(res.errCd == "0") {
					var result = res.result;

					var html = $("#list_" + themaModify.selectIndex).html();
					...
					$("#list_" + themaModify.selectIndex).html(html);
				} else {
					messageAlert.open("알림", res.errMsg);
				}
			},
			...
			onSuccess : function(status, res) {
				if(res.errCd == "0") {
					...
					if(result.is_member_login && result.register_id == result.login_id) {
						var html = $("#list_" + themaModify.selectIndex).html();
						html += "<div>";
						html += "<button id='motidy_" + themaModify.selectIndex + "' ";
						html += "onclick='javascript:themaModify.setThemaData()' style='cursor:pointer;'>";
						html += "수정하기";
						html += "</button>";
						html += "<button id='delete_" + themaModify.selectIndex + "' ";
						html += "onclick='javascript:themaModify.confirmDelete()' style='cursor:pointer;'>";
						html += "삭제하기";
						html += "</button>";
						html += "</div>";
				...
				themaModify.getThemaListsReply();
			},
			onFail : function(status) {
			}
		});
