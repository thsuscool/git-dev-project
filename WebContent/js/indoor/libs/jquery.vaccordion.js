...
//			current			: $indoorMapView.ui.baseFloor,
...
//					console.log('navigate slide: ', dir);
					aux.slide( dir, $slices, $navNext, $navPrev, settings );
				}	
			},
			slide			: function( dir, $slices, $navNext, $navPrev, settings ) {
				...
				if( cache.idx_expanded === -1 || !settings.savePositions ) {
					if( dir === 1 && settings.baseFloor + settings.visibleSlices >= cache.totalSlices )
						return false;
					else if( dir === -1 && settings.baseFloor === 0 )
						return false;
					
				
					if( dir === -1 && settings.baseFloor === 1 ) {
						$navPrev.fadeIn();
					}
					if( dir === 1 && settings.baseFloor + settings.visibleSlices === cache.totalSlices - 1 ) {
						$navNext.fadeIn();	
					}
				}
				else {
			...
			canSlideUp		: function( $slices, settings ) {
				var $first			= $slices.eq( settings.baseFloor );

				if( $first.index() !== 0 )
					return true;
			},
			canSlideDown	: function( $slices, settings ) {
				var $last			= $slices.eq( settings.baseFloor + settings.visibleSlices - 1 );
...
//							console.log('$slices.bind');
							if( settings.visibleSlices > 1 && ($(this).children('.va-title .h3').context.id != 'va-none') ) {
...
//							console.log('$navNext.bind');
							aux.navigate( 1, $slices, $navNext, $navPrev, settings );
						});
						
						$navPrev.bind('click.vaccordion', function(e){
//							console.log('$navPrev.bind');
							aux.navigate( -1, $slices, $navNext, $navPrev, settings );
						});
						
						// adds events to the mouse
						$el.bind('wheel.vaccordion', function(e, delta) {
//							console.log('mousewheel.vaccordion e: ', e);
//							console.log('mousewheel.vaccordion delta: ', e.originalEvent.wheelDelta);
