    ...
    $(".scrollBox, .dataSideScroll, .scrolls, .mapResultList").mCustomScrollbar({
    	...
    	callbacks: {
    		whileScrolling:function() {
				...
				if($("#mCSB_5_container").position().left >= 1000) {
...
/*function userColorSetting(){
	...
	$("body").on("click",".tabs.only>a",function(){
		...
		if(inx==0){
			...
			//단계설정후 레벨이 바로 반영 되지 않아 탭 변경시 반영 되도록 추가
			legendColor("#666", "#890e4f", ".colorSettingList01", $(".lvSelect").val());
		}else if(inx==1){
...
//최재영 추가
//색상설정을 누를시 레벨이 변경 되도록 변경
/*
function lvLegendSetting(){
	var color;
	var listLegnth = $(".lvSelect").val();
	var colorckList = $("#colorSetting > li> a");
	...
	legendColor("#ccc",color, ".colorSettingList01", $(".lvSelect").val());
...
//	});
	...
	*/
	//대메뉴 클릭 시
	/*body.on("click", ".qmdl dd ul>li>span", function(){   
		var inx = $(this).parent("li").index();
		var titleText = $(this).text();
		var qm = $(this).parents("ul").eq(0).attr("class");
		$(".quickBox.step02 .subj>span").text(titleText);
	...
	/*body.on("click", "#buttonMakeBtn", function(){    //3depth메뉴 나오게 하기
...
//    });
	...
	//주요지표 목록보기 - 주요지표 선택하기
//	body.on("click","#stepBox01 label",function(){
//		$(".qmdl dd ul>li").removeClass("on");
//		$interactiveLeftMenu.ui.addSearchBtn();
//		stepCloseAnimate(1);
//    });
