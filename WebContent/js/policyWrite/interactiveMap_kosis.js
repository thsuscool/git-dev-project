...
(function(W, D) {
	...
	$(document).ready(
			function() {	
				if(policyTarget==="$interactiveMap"){
					/** ********* kosis start ********* */
					//KOSIS 메뉴설정
	...
	interactiveMapKosis = {
			...
			kosisSelectedTitle : [],//2016.11.23 추가
...
//				}
					...
					case 2:
						if(admCdLen > 2) {
							if (map.isFixedBound) {
								...
								messageConfirm.open(
										"알림",
										"현재 데이터는 시군구 단위의 통계입니다.</br>지도를 시군구레벨로 축소하여 보시겠습니까?",
										btns = [
												{
												    title : "확인",
												    fAgm : null,
					...
					case 3:
						if(admCdLen > 5) {
							if (map.isFixedBound) {
								...
								messageConfirm.open(
										"알림",
										"현재 데이터는 읍면동 단위의 통계입니다.</br>지도를 읍면동레벨로 축소하여 보시겠습니까?",
										btns = [
												{
												    title : "확인",
												    fAgm : null,
			...
			setKosisParams : function() {
				...
				var _sCnt = $interactiveLeftMenu.ui.searchbtnCnt - 1;//변경
				var id = "kosis-"+_sCnt;//변경
				// 더블클릭 시, 콜백 호출
				$interactiveMap.callbackFunc.didMapDoubleClick(id, $interactiveLeftMenu.ui.arParamList[_sCnt]);//변경
				$psmCombine.ui.api.setParams("kosis",true);
...
//					interactiveMapKosis.kosisClick(result.resultdata, $(this).parent().index());
//				});
...
//				var html = "/view/map/interactiveMap/kosis?params=kosis_tb_id=" + interactiveMapKosis.kosis_search_result_data[index].kosis_menu_id + 
//					"&gis_se=" + interactiveMapKosis.kosis_search_result_data[index].gis_se 
//					+ "&kosis_org_id=" + interactiveMapKosis.kosis_search_result_data[index].kosis_inst_cd;
//				
////				$interactiveMapApi.request.searchReverseGeoCode('kosis', '0', data[i].kosis_menu_nm)
//				interactiveMapKosis.openApiReverseGeoCode('kosis', html, interactiveMapKosis.kosis_search_result_data[index].kosis_menu_nm);
			},
		...
		$class("kosis.serviceApi.kosisDataList.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res, options) {
				...
				if(res.errCd == "0") {
					...
					var preCombineCallback = $interactiveMap.ui.mapList[0].combineCallback;
					$interactiveMap.ui.mapList[0].combineCallback = function(){
						var curPolygonCode = W[policyTarget].ui.mapList[0].curPolygonCode;
						if(curPolygonCode=="4"){
							$interactiveMap.ui.curAdmCode = W[policyTarget].ui.mapList[0].curSidoCd+W[policyTarget].ui.mapList[0].curSiggCd;
						}else if(curPolygonCode=="4"){
							$interactiveMap.ui.curAdmCode = W[policyTarget].ui.mapList[0].curSidoCd+W[policyTarget].ui.mapList[0].curSiggCd+W[policyTarget].ui.mapList[0].curDongCd;
						}else{
							$interactiveMap.ui.curAdmCode = W[policyTarget].ui.mapList[0].curSidoCd;
						}
						$interactiveMap.ui.mapList[0].combineCallback = preCombineCallback;
						if($interactiveMap.ui.curDataSelect=="left"){
							var center = $interactiveMap.ui.mapList[0].gMap.getCenter();
							$interactiveLeftMenu.ui.moveInfo = {
								center : [center.x,center.y],
								zoom : $interactiveMap.ui.mapList[0].gMap.getZoom()
							};
