		...
		map : function () {
			...
			this.didMouseOverPolygon = function(event, data, type) {
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;width:30%;font-size:12px;">&nbsp;&nbsp;&nbsp;1997~2011년 화재 1000건당 사망자 증감률</th>';
					...
					this.gMap.on("zoomend", function (e) {
						...
						// 동단위 경계표출
							...
							if (that.curPolygonCode != mapPolygonCode) {
								...
								if (!that.isInnerMapShow) {
									if (that.isDrop) {
										that.openApiBoundaryHadmarea(that.curDropCd, "2011", "1");
									}
								}
							}
							that.curPolygonCode = mapPolygonCode;
					...
					if (!that.isInnerMapShow) {
						...
						case 1:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == null && that.lastGeojsonInfo.info == "sidoArea") {
								...
								that.MainObj.request.AreaAndStatsApis(null, "2011");
								console.log('전국');
						...
						case 2:
							// 최근경계조회조건 하나를 가지고 있다가 비교하여 같으면 기존에 저장된 경계를 호출
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd && that.lastGeojsonInfo.year == "2011") {
								that.MainObj.Popup.close();
								if (that.dataGeojson == null) {
									that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
								}
								...
								that.MainObj.request.AreaAndStatsApis(curSidoCd, "2011");
								console.log('시군구');
						...
						// 동면읍
						case 3:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd && that.lastGeojsonInfo.year == "2011") {
								that.MainObj.Popup.close();
								if (that.dataGeojson == null) {
									that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
						...
						// 단일동	
						case 4:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd + curDongCd && that.lastGeojsonInfo.year == "2011") {
								that.MainObj.Popup.close();
								if (that.dataGeojson == null) {
									that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
