		...
		map : function () {
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;font-size:11px;">&nbsp;&nbsp;20/30대 1인가구 여성인구</th>';
					...
					html += '</tr>';
					html += '<tr>';
					html += ' <th style="text-align: left; word-break:break-all;white-space: nowrap;padding-bottom:5px;font-size:11px;">&nbsp;&nbsp;치안시설 1개당 인구 수</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].pop_100_per_peace_fac_cnt) + '&nbsp;명';
					html += '</td>';
					html += '</tr>';
					html += '</table>';
					...
					html += '<td style="text-align:center;">& nbsp;';					
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
