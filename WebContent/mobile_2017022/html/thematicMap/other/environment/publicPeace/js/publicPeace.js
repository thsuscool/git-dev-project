...
//				$thematicMap.ui.markerGroup.clearLayers();
	...
	$thematicMap.request = {
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				...
				$thematicMap.request.getThemaWomanPeaceFacList('2010', adm_cd);
	...
	$thematicMap.response = {
		...
		successThemaWomanPeaceFac : function (status, res) {
			if (res.errCd === 0) {
				...
				if (poiList.length > 0) {
					for ( var i = 0; i < poiList.length; i++) {
						var _markerIcon = sop.icon({
							iconUrl : '/mobile/img/marker/marker/60_03.png',
...
//						_marker.addTo($thematicMap.ui.markerGroup);
						...
						if(poiList[i].corp_nm.replace(/ /gi,'')!=''){
						html += '<tr>';
