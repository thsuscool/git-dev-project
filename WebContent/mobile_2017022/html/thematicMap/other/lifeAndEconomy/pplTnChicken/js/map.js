		...
		map : function () {
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;font-size:11px;">&nbsp;&nbsp;치킨집수 </th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].chicken_corp_cnt) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += ' <th style="text-align: left; word-break:break-all;white-space: nowrap;padding-bottom:5px;font-size:11px;">&nbsp;&nbsp;치킨집 1개당 인구</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
