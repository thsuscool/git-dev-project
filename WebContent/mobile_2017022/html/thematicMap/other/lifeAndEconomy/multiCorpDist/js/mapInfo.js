		...
		mapInfo : function (map) {
			...
			this.createPoiInfo = function () {
				...
				poiInfo.update = function (props) {
					...
					html += '<img src="/mobile/img/marker/thema_marker_defailt.png">';
					html += '<p>주택유지<br>보수업체</p>';
					html += '</div>';
					 html += '</div>';
			...
			this.createBarChart = function(_position) {
				...
				//graph layer
				/*$(top.document).find(".MapArea>.Btn_Bottom>nav>.Btn_Bottom4").removeClass("NoneAction");
				$(top.document).find(".MapArea>.Btn_Bottom>nav>.Btn_Bottom4").click(function(){
					$("#barChart_"+that.id).find(".graphbox_bar").toggle();
					});*/		
					};
			// 시계열 설정 추가
