			...
			this.didMouseOverPolygon = function (event, data, type) {
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					if (that.thisObj.mapId == 'leftMap') {
						houseType = "자가";
						showdata = data.info[0].self_per;
					}
					else {
						houseType = "전세";
						showdata = data.info[0].lease_per;
					}
					...
					html += '</tr>';
					html += '<tr>';
//					html += '<th style="word-break:break-all;width:40%;font-size:12px;">&nbsp;&nbsp;&nbsp;'+year+'&nbsp;환경배출량</th>';
					html += '<th style="text-align: left; word-break:break-all;font-size:12px;">&nbsp;&nbsp;' + houseType + ' 점유비율</th>';
					html += '<td style="text-align:center; padding-left:15px;">&nbsp;';
					
					html += appendCommaToNumber(showdata) + '&nbsp;' + '(%)';
...
//					event.target.tooltip.updateZIndex(900000);
					...
					var html = '<table style="text-align:left;width:200px;white-space: nowrap;word-break:break-all;padding:5px;" >';
					html += '<tr>';
