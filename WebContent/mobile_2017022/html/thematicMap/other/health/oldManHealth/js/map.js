		...
		map : function () {
			...
			this.didMouseOverPolygon = function (event, data, type) {			
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;font-size:11px;">&nbsp;&nbsp;65세이상 노인인구 </th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].aged_psn_ppltn) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += ' <th style="text-align: left; word-break:break-all;white-space: nowrap;padding-bottom:5px;font-size:11px;">&nbsp;&nbsp;보건시설 1개당 인구</th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0].aged_pop_100_per_heal_fac_cnt) + '&nbsp;(명/개)';
//					if(data.info[0].corp_no < 1) {
//						html += '없음';
//					} else {
//						var tempVal = (data.info[0].aged_person_ppltn / data.info[0].corp_no).toString();
