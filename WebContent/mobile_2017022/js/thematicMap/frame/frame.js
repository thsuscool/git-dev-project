...
(function (W, D) {
	...
	$thematicMap.allStaticJsonObj = null;
	...
	$thematicMap.ui = {
		...
		initialize : function () {
			...
			if($sgisMobileThematicFrame.mapInfo.thema_map_type=="05"){
				$("body").on("change","#base_year>select",function(){
					var sopAbsApi = new sop.portal.absAPI();
					sopAbsApi.onBlockUIPopup();
					$thematicMap.ui.sMap.bnd_year = $(this).val();
					$thematicMap.ui.sMap.changeStatsInfo(function(){
		...
		createMap : function (centerPos, zoomlevel) {
			/* 최소 MIN 레벨 설정 */
			var maxLevel = 13;
			if($sgisMobileThematicFrame.mapInfo.max_expnsn_level=="01"){//시도
				maxLevel = 3;
			}else if($sgisMobileThematicFrame.mapInfo.max_expnsn_level=="02"){//시군구
				maxLevel = 6;
			}else if($sgisMobileThematicFrame.mapInfo.max_expnsn_level=="03"){//읍면동
				maxLevel = 8;
			}
			...
			if (!this.isMapCreated) {
				...
				if(/0(4|5)/.test($sgisMobileThematicFrame.mapInfo.thema_map_type)){
					this.sMap.mapInfo.createBarChart();
					$(top.document).find(".Btn_Top>nav>.Btn_Top4").addClass("NoneAction");
				}
			...
			$thematicMap.ui.sMap.createMapSettingBox();//주제도 설정
		...
		setMapzoomLevel : function (zoomlevel, adm_cd, callbackApi) {
			...
			if (zoomlevel >= 0 && zoomlevel <= 3) {
				...
				$thematicMap.request.sidoArea($thematicMap.ui.sMap.bnd_year, callbackApi);
					...
					layer.on({
						click : function (event) {
							...
							if(/0(4|5)/.test($sgisMobileThematicFrame.mapInfo.thema_map_type)){
								...
								var dataObject = {};
								var dataList;
								if($sgisMobileThematicFrame.mapInfo.thema_map_type=="04"){
									$.ajax({
										type : "GET",
										url : contextPath+"/ServiceAPI/thematicMap/GetThemaMapChange.json",
										data : {
											thema_map_data_id : $sgisMobileThematicFrame.mapInfo.thema_map_data_id,
											adm_cd : feature.properties.adm_cd
										},
										async: false,
										dataType:"json",
										success : function (res) {
											dataList = res.result.detailInfo;
										},
										error : function (e) {}
									});
								}else{
									dataList = $thematicMap.allStaticJsonObj;
								}
								$.each(dataList,function(cnt,node){
									var is04 = $sgisMobileThematicFrame.mapInfo.thema_map_type=="04";
									//04 증감
									//05 시계열
									var year = is04?node.irds_year:node.base_year;
									var data = is04?"chart_value":$("#stat_sel a.on").data("id")+"_data_val";
								...
								$thematicMap.ui.sMap.mapInfo.updateBarChart(feature.properties.adm_nm,category,dataArray);
							}
						}
					});
				}
	...
	/** ********* API 요청 ********* */
	$thematicMap.request = {
		getStats : function(adm_cd,callback){
			var param = {
			...
			if($sgisMobileThematicFrame.mapInfo.thema_map_type!="05"){
				param.stat_data_base_year=$thematicMap.ui.sMap.bnd_year;
			}
			$statsPotal.api.thematicMap.getStats({
				param : param,
				method : 'GET',
				success : $thematicMap.response.successGetStats,
				options : {
					callback : callback,
					param : param
				}
		...
		hadmArea : function (adm_cd, callbackStatsApi) {
			...
			$statsPotal.api.boundary.hadmarea({
				param : {
					year : $thematicMap.ui.sMap.bnd_year,
					adm_cd : adm_cd,
					low_search:"1",
					borough:"1"
				},
		...
		AreaAndStatsApis : function (_adm_cd, year) {
			...
			$thematicMap.ui.setMapzoomLevel($thematicMap.ui.map.getZoom(), adm_cd, function () {
				var sopAbsApi = new sop.portal.absAPI();
				sopAbsApi.onBlockUIPopup();
				$thematicMap.request.getStats(adm_cd,function(){
					sopAbsApi.onBlockUIClose();
				});
			});
	...
	$thematicMap.response = {
		successGetStats : function (status, res, options) {
			if (res.errCd === 0) {
				$thematicMap.staticJsonObj = [];
				if($sgisMobileThematicFrame.mapInfo.thema_map_type=="04"){
					$thematicMap.staticJsonObj.push(res.result.detailInfo);
				}else if($sgisMobileThematicFrame.mapInfo.thema_map_type=="05"){
					$thematicMap.allStaticJsonObj = res.result.detailInfo;
					var data = [];
					$.each(res.result.detailInfo,function(cnt,node){
						if(node.base_year==$thematicMap.ui.sMap.bnd_year){
							data.push(node);
						}
					});
					$thematicMap.staticJsonObj.push(data);
				}else{
					$thematicMap.allStaticJsonObj = res.result.detailInfo;
					$thematicMap.staticJsonObj.push(res.result.detailInfo);
				}
				$thematicMap.formatter.combinStatsToGeoJon($thematicMap.geoJsonObj, $sgisMobileThematicFrame.mapInfo[$("#stat_sel a.on").data("id")+"_sep_unit"], $("#stat_sel a.on").data("id")+'_data_val');
			} else {
