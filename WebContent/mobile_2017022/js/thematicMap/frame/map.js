		...
		map : function () {
			...
			this.didMouseOverPolygon = function(event, data, type) {
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					...
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;padding:5px;font-size:12px;"><strong>'+data.properties.adm_nm+'</strong></th>';
					html += '<td style="text-align:center;">';					
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;font-size:11px;">&nbsp;&nbsp;'+data.info[0].base_year+'년 '+$sgisMobileThematicFrame.mapInfo[$("#stat_sel a.on").data("id")+'_sep_ttip_title']+': </th>';
					html += '<td style="text-align:center; padding-left:15px;">';
					html += appendCommaToNumber(parseFloat(data.info[0][data.info[0].showData])) + '&nbsp;'+data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '</table>';
					...
					var html = '<table style="text-align:left;white-space: nowrap;word-break:break-all;" >';
					html += '<tr>';
					html += '<th style="word-break:break-all;padding:5px;font-size:12px;"><strong>'+data.properties.adm_nm+'</strong></th>';
					html += '<td style="text-align:center;">&nbsp;';					
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += '<th style="word-break:break-all;padding-bottom:5px;font-size:11px;">&nbsp;데이터가 없습니다.</th>';
					html += '<td style="text-align:center;">';					
					html += '</td>';
					html += '</tr>';
					html += '</table>';
								...
								for ( var i = 0; i < that.data[k].result.length; i++) {
									for (key in that.data[k].result[i]) {
										if (key == that.data[k].showData) {
											tmpData.push(that.data[k].result[i][key]);
											break;
										}
									}
								}
			...
			this.createMapSettingBox = function(){
				...
				settingbox.onAdd = function(map) {
					...
					var html = '';
					html+='	<div id="setting_control" class="control_item setting_control" style="'+((hasLeft&&hasRight)||$sgisMobileThematicFrame.mapInfo.thema_map_type=="05"?'':'display:none;')+'"></div>';
					html+='	<div class="sqListBox sq03" style="left: -380px;top:0px;'+($sgisMobileThematicFrame.mapInfo.thema_map_type=="05"?"height: 100px;":"")+'">';
					html+='		<div class="sqList" style="'+($sgisMobileThematicFrame.mapInfo.thema_map_type=="05"?"height: 100px;":"")+'">';
					html+='			<ul>';
					html+='				<li id="stat_sel">';
					html+='					<span>통계선택</span>';
					if (hasLeft){
						html+='<a href="#" class="on" data-id="left">'+$sgisMobileThematicFrame.mapInfo.left_sep_nm+'('+$sgisMobileThematicFrame.mapInfo.left_sep_unit+')</a>';
					}
					if (hasRight){
						html+='<a href="#" class="'+(!hasLeft?"on":"")+'" data-id="right">'+$sgisMobileThematicFrame.mapInfo.right_sep_nm+'('+$sgisMobileThematicFrame.mapInfo.right_sep_unit+')</a>';
					}
					html+='				</li>';
...
//							}
							var selected = (cnt==yearList.length-1);
							if(selected){
								that.bnd_year = node.base_year;
							}
			...
			this.changeStatsInfo = function(callback){
				...
				if (zoomlevel >= 0 && zoomlevel <= 3) {
					...
					$thematicMap.request.sidoArea(that.bnd_year,function(){
						$thematicMap.request.getStats(null,callback);
					});
				}
				else if (zoomlevel > 3 && zoomlevel <= 5) {
					// 시군구 hadmarea
					$thematicMap.ui.sMap.curPolygonCode = 2;
					$thematicMap.request.hadmArea(that.curSidoCd, function(){
						$thematicMap.request.getStats(that.curSidoCd,callback);
					});
				}
				else if (zoomlevel > 5 && zoomlevel <= 8) {
					// 동경계 경계 hadmarea
					$thematicMap.ui.sMap.curPolygonCode = 3;
					$thematicMap.request.hadmArea(that.curSidoCd+that.curSiggCd, function(){
						$thematicMap.request.getStats(that.curSidoCd+that.curSiggCd,callback);
					});
				}
			};
		}
					...
					if (!that.isInnerMapShow) {
						...
						case 1:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == null && that.lastGeojsonInfo.info == "sidoArea") {
								...
								that.MainObj.request.AreaAndStatsApis(null, that.bnd_year);
							}
							break;
						...
						case 2:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd && that.lastGeojsonInfo.year == that.bnd_year) {
								...
								that.MainObj.request.AreaAndStatsApis(curSidoCd, that.bnd_year);
							}
							break;
						...
						case 3:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd && that.lastGeojsonInfo.year == that.bnd_year) {
								...
								// Api 요청
								that.MainObj.request.AreaAndStatsApis(curSidoCd + curSiggCd, that.bnd_year);
							}
							break;											
