...
(function (W, D) {
	...
	sMap = {
		...
		map : function () {
			...
			this.createInfoControl = function (val, type) {
				var infoControl;
				
				if(type == 'rightMap') {
					infoControl = sop.control({
						position : 'topleft'
					});
				} else {
					infoControl = sop.control({
			...
			this.createLeftDataWindow = function (type) {
				var leftWindow;
				
				if(type == 'rightMap') {
					leftWindow = sop.control({
						position : 'topleft'
					});
				} else {
					leftWindow = sop.control({
				...
				leftWindow.onAdd = function (map) {
					this._div = sop.DomUtil.create('div', 'leftWindow');
					sop.DomEvent.disableClickPropagation(this._div);
					this.update();
					return this._div;
				};
				
				leftWindow.update = function (props) {
					if(props != null && props != undefined) {
						this._div.innerHTML = props;
					}
				};
				this.leftWindow = leftWindow;
				leftWindow.addTo(this.gMap);
				
				$(".leftWindow").css("font", "12px/14px Arial, Helvetica, sans-serif").css("background", "white").css("background", "rgba(255,255,255,1)").css("box-shadow", "0 0 15px rgba(0,0,0,0.2)").css("border-radius", "5px").css("opacity", "0.8");
			},
			...
			this.didMouseOverPolygon = function (event, data, type) {
				if (!sop.Util.isUndefined(data.info) && data.info.length > 0) {
					var showdata = "";
					var showText = showText = data.info[0].base_year+"년 "+data.info[0].dataName+" :";
					var html = '<table style="text-align:left;white-space: nowrap;word-break:break-all;" >';
					html += '<tr>';
					if(data.properties.adm_nm != "null") {
						html += '<th style="text-align: left; word-break:break-all;padding:5px;font-size:14px;"><strong>'+data.properties.adm_nm+'</strong></th>';
					} else {
						html += '<th style="text-align: left; word-break:break-all;padding:5px;font-size:14px;"><strong>집계구</strong></th>';
					}
					html += '<td style="text-align:center;">';					
					html += '</td>';
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;font-size:12px;">&nbsp;&nbsp;' + showText + ' </th>';
					html += '<td style="text-align:right; padding-left:15px;">';
					html += appendCommaToNumber(data.info[0][data.info[0].showData]) + '&nbsp;' + data.info[0].unit;
					html += '</td>';
					html += '</tr>';
					html += '</table>';
					
					if (that.leftWindow != null) {
						that.leftWindow.update(html);
					}
					that.thisObj.sMap.mapInfo.selectChartData(data.properties, data.dataIdx);
					
					that.thisObj.sMap.mapInfo.selectLegendRangeData(event.target.options.fillColor);
				}
				else {
					var html = '<table style="text-align:left;white-space: nowrap;word-break:break-all;" >';
					html += '<tr>';
					...
					html += '</tr>';
					html += '<tr>';
					html += '<th style="text-align: left; word-break:break-all;font-size:12px;">&nbsp;데이터가 없습니다.</th>';
					html += '<td style="text-align:center;">';
					html += '</td>';
					html += '</tr>';
					html += '</table>';
			...
			this.didSelectedPolygon = function (event, data, type) {
				// 차트 하이라이트 효과
				this.thisObj.sMap.mapInfo.selectChartData(data.properties);
				// 범례 하이라이트 효과
				this.thisObj.sMap.mapInfo.selectLegendRangeData(event.target.options.fillColor);
			};
							...
							if (feature.info[x].showData) {
								for (param in feature.info[x]) {
									if (param == feature.info[x].showData) {
										if(that.valPerSlice[x] != undefined) {
											layer.setStyle({
			...
			this.calculateLegend = function (arData) {
				...
				for ( var k = 0; k < arData.length; k++) {
					...
					if (tmpResult.length < 5) {
						for ( var j = 0; j < 5 - tmpResult.length; j++) {
							tmpResult.push("");
						}
					}
					...
					sop.DomEvent.on(this._div, 'click', function() {
						...
						if (division.hasClass("upscale")) {
							division.removeClass("upscale").addClass("downscale");
								...
								$(top.document).find(".MapArea,.Map,#themeticFrame").height($(top.document).height());
							...
							division.removeClass("downscale").addClass("upscale");
                                ...
                                $(top.document).find(".MapArea,.Map,#themeticFrame").height($(window).outerHeight(true)-($(top.document).find(".Wrap>.Header").outerHeight(true)+$(top.document).find(".Content>.Btn_Top").outerHeight(true)+$(top.document).find(".SelectArea").outerHeight(true)));
			...
			this.createMapSettingBox = function(){
				var hasLeft = $sgisMobileThematicFrame.mapInfo.left_sep_nm !=null && $sgisMobileThematicFrame.mapInfo.left_sep_unit !=null && $sgisMobileThematicFrame.mapInfo.left_sep_nm.length > 0 && $sgisMobileThematicFrame.mapInfo.left_sep_unit.length > 0; 
				var hasRight = $sgisMobileThematicFrame.mapInfo.right_sep_nm !=null && $sgisMobileThematicFrame.mapInfo.right_sep_unit!=null && $sgisMobileThematicFrame.mapInfo.right_sep_nm.length > 0 && $sgisMobileThematicFrame.mapInfo.right_sep_unit.length > 0;
				var settingbox = sop.control({
					position: 'topleft'
				});
				var that = this;
				settingbox.onAdd = function(map) {
					this._div = sop.DomUtil.create('div', 'mapSetting');
					var max_expnsn_level = isNaN(parseInt($sgisMobileThematicFrame.mapInfo.max_expnsn_level))?0:parseInt($sgisMobileThematicFrame.mapInfo.max_expnsn_level);
					var html = '';
					html+='	<div id="setting_control" class="control_item setting_control" style="'+(hasLeft&&hasRight?'':'display:none;')+'"></div>';
					html+='	<div class="sqListBox sq03" style="left: -380px;top:0px;'+($sgisMobileThematicFrame.mapInfo.thema_map_type=="05"?"height: 270px;":"")+'">';
					html+='		<div class="sqList" style="'+($sgisMobileThematicFrame.mapInfo.thema_map_type=="05"?"height: 270px;":"")+'">';
					html+='			<ul>';
					html+='				<li id="stat_sel">';
					html+='					<span>통계선택</span>';
					if (hasLeft){
						html+='<a href="#" class="on" data-id="left">'+$sgisMobileThematicFrame.mapInfo.left_sep_nm+'('+$sgisMobileThematicFrame.mapInfo.left_sep_unit+','+$sgisMobileThematicFrame.mapInfo.sep_map_left_sep_unit+')</a>';
					}
					if (hasRight){
						html+='<a href="#" class="'+(!hasLeft?"on":"")+'" data-id="right">'+$sgisMobileThematicFrame.mapInfo.right_sep_nm+'('+$sgisMobileThematicFrame.mapInfo.right_sep_unit+','+$sgisMobileThematicFrame.mapInfo.sep_map_right_sep_unit+')</a>';
					}
					html+='				</li>';
					if($sgisMobileThematicFrame.mapInfo.thema_map_type=="05"){
						html+='				<li id="base_year">';
						html+='					<span>년도선택</span>';
						html+='					<select class="original">';
						var yearList = $sgisMobileThematicFrame.getThemeBaseYesr($sgisMobileThematicFrame.mapInfo.thema_map_data_id);
						$.each(yearList,function(cnt,node){
							var selected = $sgisMobileThematicFrame.mapInfo.stat_data_base_year==node.base_year;
							if(selected){
								that.bnd_year = node.base_year;
							}
							html+='<option '+(selected?"selected='selected'":"")+'>'+node.base_year+'</option>';
						});
						html+='					</select>';
						html+='				</li>';
					}
					html+='			</ul>';
					html+='		</div> ';
					html+='		<a href="javascript:void(0)" class="stepClose2" onclick="$sgisMobileThematicFrame.closeSetting();"></a>';
					html+='	</div>';
					$(this._div).append(html);
					sop.DomEvent.disableClickPropagation(this._div);
					this.update();
					$(this._div).attr("id", 'mapSettingBox' + that.id).attr("title","주제도설정");
					return this._div;
				};
				
				settingbox.update = function() {
					$(this._div).find("#setting_control").click(function(){
						if($(this).hasClass("on")){
							$sgisMobileThematicFrame.closeSetting();
						}else{
							$sgisMobileThematicFrame.openSetting();
						}
					});
					$(this._div).find("#stat_sel a").click(function(){
						$("#stat_sel a").removeClass("on");
						$(this).addClass("on");
						var sopAbsApi = new sop.portal.absAPI();
						sopAbsApi.onBlockUIPopup();
						that.changeStatsInfo(function(){
							sopAbsApi.onBlockUIClose();
						});
					});
				};
				settingbox.addTo(this.gMap);
			};
			this.changeStatsInfo = function(callback){
				$thematicMap.leftMap.sMap.leftWindow.update("");
				$thematicMap.rightMap.sMap.leftWindow.update("");
				var zoomlevel = that.gMap.getZoom();
				var setting = function(obj,type,curPolygonCode,adm_cd,callback){
					$thematicMap[type].sMap.curPolygonCode = 1;
					obj($thematicMap[type].sMap.bnd_year,function(){
						$thematicMap[type].BoundaryApi.getStats($thematicMap[type], adm_cd,callback);
					},$thematicMap[type]);
				};
				if (zoomlevel >= 0 && zoomlevel <= 3) {
					// 전국시도경계
					setting($thematicMap.leftMap.BoundaryApi.sidoArea,"leftMap",1,null,callback);
					setting($thematicMap.rightMap.BoundaryApi.sidoArea,"rightMap",1,null,callback);
				}
				else if (zoomlevel > 3 && zoomlevel <= 5) {
					// 시군구 hadmarea
					setting($thematicMap.leftMap.BoundaryApi.hadmArea,"leftMap",2,$thematicMap.leftMap.curSidoCd,callback);
					setting($thematicMap.rightMap.BoundaryApi.hadmArea,"rightMap",2,$thematicMap.leftMap.curSidoCd,callback);
				}
				else if (zoomlevel > 5 && zoomlevel <= 8) {
					// 동경계 경계 hadmarea
					setting($thematicMap.leftMap.BoundaryApi.hadmArea,"leftMap",3,$thematicMap.leftMap.curSidoCd+$thematicMap.leftMap.curSiggCd,callback);
					setting($thematicMap.rightMap.BoundaryApi.hadmArea,"rightMap",3,$thematicMap.rightMap.curSidoCd+$thematicMap.rightMap.curSiggCd,callback);
				}
			};
		}
		...
		$class("sop.openApi.ReverseGeoCode.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				...
				if (res.errCd == "0") {
					...
					if (!that.isInnerMapShow) {
						...
						case 1:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == null && that.lastGeojsonInfo.info == "sidoArea") {
								...
								that.thisObj.request.AreaAndStatsApis(null, "2012", target);
							}
							break;
						...
						case 2:
							...
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd && that.lastGeojsonInfo.year == "2012") {
								...
								that.thisObj.request.AreaAndStatsApis(curSidoCd, "2012", target);
							}
							break;
						...
						case 3:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd && that.lastGeojsonInfo.year == "2012") {
								...
								// Api 요청
								that.thisObj.request.AreaAndStatsApis(curSidoCd + curSiggCd, '2012', target);
							}
							break;
						...
						case 4:
							if (that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd + curSiggCd + curDongCd && that.lastGeojsonInfo.year == "2012") {
								...
								that.thisObj.request.AreaAndStatsApis(curSidoCd + curSiggCd + curDongCd, '2012', target);
							}
							break;
						default:
							break;
					...
					if (that.thisObj.mapId == 'leftMap') {
						that.thisObj.mapNavi1.reverseOnSelectChange(that);
					}
