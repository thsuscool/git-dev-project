...
(function (W, D) {
	...
	sMapInfo = {
		mapInfo : function (map) {
			...
			// 막대차트 업데이트
			this.updateBarChart = function(adm_nm, xAxisCat, retDataList) {
				...
				var title = $sgisMobileThematicFrame.mapInfo[$("#stat_sel a.on").data("id")+"_sep_chart_title"];
				...
				if (_valPerSlice != undefined) {
					...
					if (point.length < 5) {
						for (var i=0; i<point.length; i++) {
							$("#legend_" + this.id).find("#legendRange" + (i+1)).text(parseFloat(point[i]));
						}
