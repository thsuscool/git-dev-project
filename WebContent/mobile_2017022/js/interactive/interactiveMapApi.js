...
(function(W, D) {
	...
	$interactiveMapApi.request = {
		...
		openApiCompany: function(params) {
			...
			var isBndYear = false;
			var url = openApiPath + this.API_0301_URL;
			for (var i = 0; i < params.param.length; i++) {
				if(params.param[i].key=="class_code"||params.param[i].key=="theme_cd"){
			...
			if(url==openApiPath + this.API_0301_URL){
				sopOpenApiCompanyObj.removeParam("class_deg");
				sopOpenApiCompanyObj.removeParam("area_type");
				if(params.filter == "tot_worker"){
					params.filter = "employee_cnt";
				}
			}
		 ...
		 */
		openApiStatisticsHistoryParamInfo: function(hist_id,options) {
		...
		openApiShareForStats: function(params) {
			...
			if (params.param_info.isKosis) {
				messageAlert.open("알림", "모바일 화면에서는 kosis정보를 확인 하실 수 없습니다");
			} else {
				//조건결합일 경우, post
				var method = "GET";
		...
		openApiStatBaseYearProcess: function() {
			var searchbtnCnt = $interactiveMap.ui.searchbtnCnt - 1;
			if (searchbtnCnt < 0) {
				searchbtnCnt = 0;
			}
			var sopStatBaseYearObj = new sop.portal.statBaseYear.api();
			var param_info = new Array();
			$.each($interactiveMap.ui.arParamList, function() {
				if (this.idx == searchbtnCnt) {
					$.each(this.params, function() {
						param_info.push(this.key);
					});
					return false;
				}
			});
			sopStatBaseYearObj.addParam("api_id", $interactiveMap.ui.curSelectedDetailStatsType.substring(0, 8));
		...
		setStatsData: function(res, options) {
			...
			if ($interactiveMap.ui.searchBtnType == "normal") {
				if (mapInfo.isTimeSeriesPlay) {
							...
							if (map.zoom < 9) {
								messageAlert.open(
									...
									function done() {
										map.setZoom(10); //10->9
										setTimeout(function() {
											map.gPoi.requestCompanyPoi(tmpParams, "0");
										}, 500);
									},
									function cancel() {}
								);
					...
					$interactiveDataBoard.ui.updateTargetAreaTable(res,options);
				}
				// 북마크,공유정보 설정
				...
				$interactiveMap.ui.setShareInfo(options, "normal", map.id);
				if (map.combineData.length == 2) {
					var shareUrlInfoList = $interactiveMap.ui.shareUrlInfoList[map.id];
					var combineTitle = "";
					var api_ids = "";
		...
		$class("sop.openApi.totalPopulation.api").extend(sop.portal.absAPI)
			.define({
				onSuccess: function(status, res, options) {
					options.params.map.mapInfo.updateSearchTitle(options);
					if (res.errCd == "0") {
						$interactiveMapApi.request.setStatsData(res, options);
					} else if (res.errCd == "-401") {
						accessTokenInfo();
						setTimeout($interactiveMapApi.request.openApiTotalPopulation(options.params), 500);
					} else {
			...
			.define({
				onSuccess: function(status, res, options) {
					...
					if (res.errCd == "0") {
						...
						accessTokenInfo();
						setTimeout($interactiveMapApi.request.openApiSearchPopulation(options.params), 500);
					} else {
						$interactiveMapApi.request.combineFailCnt++;
						var map = options.params.map;
						map.clearDataOverlay();
			...
			.define({
				onSuccess: function(status, res, options) {
					...
					if (res.errCd == "0") {
						...
						accessTokenInfo();
						setTimeout($interactiveMapApi.request.openApiHouseHold(options.params), 500);
					} else {
						$interactiveMapApi.request.combineFailCnt++;
						var map = options.params.map;
						map.clearDataOverlay();
			...
			.define({
				onSuccess: function(status, res, options) {
					...
					if (res.errCd == "0") {
						...
						accessTokenInfo();
						setTimeout($interactiveMapApi.request.openApiHouseHoldMember(options.params), 500);
					} else {
						$interactiveMapApi.request.combineFailCnt++;
						var map = options.params.map;
						map.clearDataOverlay();
			...
			.define({
				onSuccess: function(status, res, options) {
					...
					if (res.errCd == "0") {
						...
						accessTokenInfo();
						setTimeout($interactiveMapApi.request.openApiFarmHouseHold(options.params), 500);
					} else {
						$interactiveMapApi.request.combineFailCnt++;
						var map = options.params.map;
						map.clearDataOverlay();
			...
			.define({
				onSuccess: function(status, res, options) {
					...
					if (res.errCd == "0") {
						...
						accessTokenInfo();
						setTimeout($interactiveMapApi.request.openApiForestryHouseHold(options.params), 500);
					} else {
						$interactiveMapApi.request.combineFailCnt++;
						var map = options.params.map;
						map.clearDataOverlay();
			...
			.define({
				onSuccess: function(status, res, options) {
					...
					if (res.errCd == "0") {
						...
						accessTokenInfo();
						setTimeout($interactiveMapApi.request.openApiFisheryHouseHold(options.params), 500);
					} else {
						$interactiveMapApi.request.combineFailCnt++;
						var map = options.params.map;
						map.clearDataOverlay();
	...
	(function() {
		$class("sop.openApi.InderstryCode.api").extend(sop.portal.absAPI)
			.define({
				onSuccess: function(status, res, options) {
					options.sopAbs.onBlockUIClose();
					var result = res.result;
					if (res.errCd == "0") {
						$("#company-list").empty();
						$.each(result, function() {
							$("#company-list").append(
								'<li data-text="' + this.class_nm + '">' +
								'<label><input name="company_list" type="radio" value="' + this.class_code + '" data-title="' + this.class_nm + '">' + this.class_nm + '</label> ' +
								($interactiveMap.ui.companyDepth < 4 ? '<button class="Btn_Next" type="button" title="다음" onclick="$interactiveMap.ui.changeCompany(this)">▶</button>' : "") +
								'</li>'
							);
						});
						$("#company-list input[name=company_list]:radio").change(function() {
							$interactiveMap.ui.curSelectedCompanyNode = {
								cd: $(this).val(),
						...
						accessTokenInfo();
						options.sopAbs.onBlockUIPopup();
						setTimeout($interactiveMapApi.request.openApiInterstryCode(
							options.depth,
							options.class_deg,
		...
		$class("sop.openApi.Company.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				...
				if (res.errCd == "0") {
					...
					accessTokenInfo();
					setTimeout($interactiveMapApi.request.openApiCompany(options.params), 500);
				} else {
					$interactiveMapApi.request.combineFailCnt++;
					var map = options.params.map;
					map.clearDataOverlay();
		...
		$class("sop.openApi.House.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				...
				if (res.errCd == "0") {
					...
					accessTokenInfo();
					setTimeout($interactiveMapApi.request.openApiHouse(options.params), 500);
				} else {
					$interactiveMapApi.request.combineFailCnt++;
					var map = options.params.map;
					map.clearDataOverlay();
				...
				if (res.errCd == "0") {
					if (result.hist_type == "SHARE") {
						...
						if (result.map_type == "IMAP") {
							linkUrl = "//"+location.host + "/mobile/html/interactive/interactiveMap.html?"
						} else {
							linkUrl = "//"+location.host + "/html/c/cMap.html?"
						}
...
//						FB.XFBML.parse(elemDiv);
						//페이스북
					} else if (result.hist_type == "BMARK") {
						messageAlert.open("알림", "북마크 등록 완료 되었습니다");
					}
				} else {
					if(location.pathname.indexOf(communityPath)!=-1){
						if(res.errCd!="-100"){
		...
		$class("sop.openApi.statisticsHistoryParamInfo.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				...
				if (res.errCd == "0") {
					...
					$interactiveMap.ui.setRevertParams(infoList, "share");
					...
					for (var i = 0; i < infoList.length; i++) {
						...
						if(options&&options.options){
							//커뮤니티 맵 개설 시 읍면동 위치 이동
							infoList[i]["param_info"]["adm_cd"] = options.options.addressAdmCd;
							infoList[i]["param_info"]["mapInfo"]["center"][0] = options.options.x;
							infoList[i]["param_info"]["mapInfo"]["center"][1] = options.options.y;
							infoList[i]["param_info"]["paramInfo"]["adm_cd"] = options.options.addressAdmCd;
						}
			...
			onSuccess: function(status, res, options) {
				...
				if (res.errCd == "0") {
					...
					if ($interactiveMap.ui.searchBtnType == "normal") {
						...
						$interactiveMap.ui.setShareInfo(options.params, "share", map.id);
						//사업체 조회일 경우
						if (res.id == "API_0304") {
					...
					accessTokenInfo();
					setTimeout($interactiveMapApi.request.openApiShareForStats(options.params), 500);
				} else {
					$interactiveMapApi.request.combineFailCnt++;
					if(location.pathname.indexOf(communityPath)!=-1){
						if(res.errCd!="-100"){
			...
			onSuccess: function(status, res, options) {
				if (res.errCd == "0") {
					...
					if (result.base_year != null) {
						...
						for (var i = 0; i < $interactiveMap.ui.arParamList.length; i++) {
							if ($interactiveMap.ui.arParamList[i].idx == searchbtnCnt) {
								tmpParamList = $interactiveMap.ui.arParamList[i];
		...
		$class("sop.openApi.geocode.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				if (res.errCd == "0") {
					...
					accessTokenInfo();
					setTimeout($interactiveMapApi.request.openApiGeocode(options.address), 500);
		...
		$class("sop.openApi.sopsearch.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				...
				if (res.errCd == "0") {
					...
					accessTokenInfo();
					setTimeout($interactiveMapApi.request.openApiSOP(options.searchword, options.pagenum), 500);
				}
			},
			onFail: function(status) {}
		});
		...
		$class("sop.openApi.kosissearch.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				...
				if (res.errCd == "0") {
					...
					accessTokenInfo();
					setTimeout($interactiveMapApi.request.openApiKOSIS(options.searchword, options.pagenum), 500);
				}
			},
			onFail: function(status) {}
		});
		...
		$class("sop.openApi.itemcombine.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				if (res.errCd == "0") {
					...
					accessTokenInfo();
					setTimeout($interactiveMapApi.request.openApiItemCombine(options.params), 500);
				} else {
					var map = options.params.map;
					//map.clearData();
					map.clearDataOverlay();
